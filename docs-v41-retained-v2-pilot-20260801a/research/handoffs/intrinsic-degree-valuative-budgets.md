...
### One-sided high-weight composition theorem

The single-shear theorem extends to genuinely noncommuting words. Let `U_+`
be generated by exponentials of homogeneous source derivations of torus
weight at least `19`, and `U_-` by weights at most `-19`. Then

```text
sigma(S) intersect B_{<=6} subset span{1,Q,R}
```

for every `sigma in U_+ union U_-`; hence `trdeg C_6^sigma<=2`.
Coefficients, word length, order, and coordinate direction are unrestricted.

The proof is a weight-window lemma. In `sigma^(-1)g`, every nonzero ordered
derivative word shifts a weight of `g in B_{<=6}` beyond `[-6,12]` because
all shifts have the same sign and magnitude greater than the window width.
The in-window components are therefore exactly those of `g`; gradedness of
`S` forces them into `S`.

This covers arbitrary noncommuting words in the positive tails

```text
x<-x+c*y^N (N>=18),   z<-z+c*y^N (N>=21),
y<-y+c*z^N (N>=10),   x<-x+c*z^N (N>=9),
```

or arbitrary words in the negative tails

```text
z<-z+c*x^N (N>=17),   y<-y+c*x^N (N>=18).
```

Arbitrary interleaving with the resonant shears `z<-z+a*y^2` is also allowed:
normal-ordering preserves derivation weights, and the exact resonant theorem
then gives transcendence degree at most one when the total resonant parameter
is nonzero. An arbitrary affine source post-factor preserves the conclusion.
...
The elementary-shear and one-sided high-weight composition theorems give the
stronger condition `J_6^sigma=0` throughout their stated source semigroups.
...
The remaining source-frame problem is now confined to compositions containing
low-weight nonlinear factors, mixed positive/negative high-weight factors
whose derivative weights can return to `[-6,12]`, or wild automorphisms.

Useful next theorems are:

1. an exact two-shear classification for the first mixed-sign or low-weight
   interaction;
2. a propagation theorem for the differential-minor obstruction through a
   tame factorization;
3. a non-degree-increasing normal form for tame source automorphisms; or
4. a concrete composition producing `trdeg C_6^sigma=3`, which would identify
   the precise failure of the current semigroup control.

The older `delta(Q)<=9` packet still needs a public definition, case inventory,
localizations, and artifact map before it can be compared to these theorems.

## Useful deliverable

Return one of:

- an exact mixed-sign or low-weight two-shear theorem;
...
- a concrete composition where the current semigroup theorem fails.
