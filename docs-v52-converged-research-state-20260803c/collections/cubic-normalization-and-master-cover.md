---
title: "Cubic Normalization And Master Cover"
description: "This current collection omits historical wording. Replaced records point to their current retained mathematics."
---

# Cubic Normalization And Master Cover

<p class="dek">This current collection omits historical wording. Replaced records point to their current retained mathematics.</p>

<span class="status status-kind">Result</span> <span class="status coverage-complete">Exact manuscript location</span>

## Precise statement

Current mathematical statements are listed unit by unit below; superseded and archival wording is not reproduced.

## Claims in this result package

### [JCG-C04524CF · Every generic-degree-three Keller map factors through a finite normal cubic cover whose…](../claims/JCG-C04524CF.md)

Every generic-degree-three Keller map factors through a finite normal cubic cover whose trace-zero module is rank-two reflexive and locally free away from finitely many target points.

!!! info "Stronger current result available"
    The statement above remains valid. A stronger current formulation is available:

    - [Canonical finite Ext defect for a cubic Keller normalization](../research/working-mathematics/units/RMU-1A8D0001.md)

*Defining · Supporting Result · Proof supplied*

### [JCG-248AD7B9 · Replaced historical record](../claims/JCG-248AD7B9.md)

!!! warning "Replaced working statement"
    The earlier wording is suppressed. Continue with the current replacement mathematics below.

- [Canonical finite Ext defect for a cubic Keller normalization](../research/working-mathematics/units/RMU-1A8D0001.md): Let R=C[y_1,y_2,y_3], let B be the finite normalization attached to a generic-degree-three Keller map, and write B=R direct-sum E by trace. Then B and E are reflexive and Delta_F=Ext^1_R(B,R)=Ext^1_R(E,R) has finite length. Its support is exactly the locus where B is not free over R. Consequently B is finite flat over R if and only if Delta_F=0. At a closed point y, a minimal resolution 0 -> A^b -> A^(b+2) -> E_y -> 0 over A=R_y presents (Delta_F)_y as the cokernel of the dual matrix, with b minimal generators.

*Defining · Supporting Result · Recorded*

### [JCG-AB57A9B2 · Replaced historical record](../claims/JCG-AB57A9B2.md)

!!! warning "Replaced working statement"
    The earlier wording is suppressed. Continue with the current replacement mathematics below.

- [Exact quadratic-resolvent carrier of the cubic flatness defect](../research/working-mathematics/units/RMU-1A8D0004.md): Let T be the normalization in the S_3 Galois closure, Q=T^(A_3), and L the nontrivial cubic eigensheaf. Then Q is a normal finite-flat quadratic R-algebra, T=Q direct-sum L direct-sum L^[2], L^[3]=Q, and the quadratic involution satisfies sigma^*L=L^[2]=L^dual. As an R-module, the cubic trace-zero summand E is isomorphic to L. Hence Delta_F=Ext^1_R(L,R), and B is finite flat over R if and only if L is maximal Cohen-Macaulay over Q.

*Defining · Supporting Result · Recorded*

### [JCG-26FD089B · A generically degree-three Keller map admits a useful normalization picture: a finite…](../claims/JCG-26FD089B.md)

A generically degree-three Keller map admits a useful normalization picture: a finite normalization X over the target together with an open embedding of A3, expressible through a finite-flat binary-cubic model.

*Defining · Partial Result · Recorded*

### [JCG-A5938472 · Replaced historical record](../claims/JCG-A5938472.md)

!!! warning "Replaced working statement"
    The earlier wording is suppressed. Continue with the current replacement mathematics below.

- [Canonical finite Ext defect for a cubic Keller normalization](../research/working-mathematics/units/RMU-1A8D0001.md): Let R=C[y_1,y_2,y_3], let B be the finite normalization attached to a generic-degree-three Keller map, and write B=R direct-sum E by trace. Then B and E are reflexive and Delta_F=Ext^1_R(B,R)=Ext^1_R(E,R) has finite length. Its support is exactly the locus where B is not free over R. Consequently B is finite flat over R if and only if Delta_F=0. At a closed point y, a minimal resolution 0 -> A^b -> A^(b+2) -> E_y -> 0 over A=R_y presents (Delta_F)_y as the cokernel of the dual matrix, with b minimal generators.

*Defining · Supporting Result · Recorded*

### [JCG-ECD1CA4E · The resultant-one factor space is isomorphic to SL2 times A1_gamma, and its…](../claims/JCG-ECD1CA4E.md)

The resultant-one factor space is isomorphic to SL2 times A1_gamma, and its universal marked-root map to the simple-root binary-cubic locus is etale; ordering the roots gives an S3 torsor.

*Defining · Supporting Result · Proof supplied*

### [JCG-BD8A0F77 · Replaced historical record](../claims/JCG-BD8A0F77.md)

!!! warning "Replaced working statement"
    The earlier wording is suppressed. Continue with the current replacement mathematics below.

- [Source splitting and the exact attained-value boundary](../research/working-mathematics/units/RMU-1A8D0003.md): For every generic-degree-three complex Keller normalization, base change to the affine source gives B tensor_R S isomorphic to S times C, where the S-factor is the canonical marked section and C is a normal quadratic S-algebra. After choosing a generator eta of its trace-zero summand, C is isomorphic to S[eta]/(eta^2-D). The generator and D are choice-dependent. The normalization is free over every attained target value, and Supp(Delta_F) is contained in the omitted-value locus, hence in the singular locus of the reduced nonproperness set.

*Defining · Proof Lemma · Proof supplied*

### [JCG-2E563312 · If the cubic normalization is flat, it is the pullback of the…](../claims/JCG-2E563312.md)

If the cubic normalization is flat, it is the pullback of the universal marked-root master cover; equality with affine three-space additionally requires completeness of the boundary deletion.

*Defining · Primary Statement · Recorded*

## Evidence and manuscript boundary

The linked working manuscript has exact locations for every program-relevant defining claim on this page. This says where the claims are recorded, not that they have been independently verified.

Complete coverage requires an exact LaTeX location for every program-relevant defining claim. A locator records where the current statement and evidence boundary appear; it is not a proof-review status.

## Credit

- Nathaniel Monson: research direction and mathematical responsibility; attributed by source

## Connections

- [Base Cover Monodromy And Deck Group](../collections/base-cover-monodromy-and-deck-group.md)
- [Base Map Fibers Image And Nonproperness](../collections/base-map-fibers-image-and-nonproperness.md)
- [The Common Cover of the Descendant Ladder](../collections/common-cover-of-the-descendant-ladder.md)
- [Invertibility and Properness for Complex Keller Maps](../collections/keller-invertibility-and-properness.md)
- [The Root-Slope Mechanism and Cubic Cover Geometry](../collections/root-slope-and-cubic-cover-geometry.md)

[Back to Results](../results/index.md)
