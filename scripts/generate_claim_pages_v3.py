...
def render_index(records: dict[str, dict[str, Any]]) -> str:
    ...
    for storyline, heading in ordered:
        ...
        for record in sorted(grouped[storyline], key=lambda item: item["id"]):
            ...
            claim_link = f"[{record['title']}](claim-v3/{record['slug']}.md)"
...
def load_record_relations(
    ...
    events_by_claim: dict[str, list[dict[str, Any]]] = {
        claim_id: [] for claim_id in records
    }
    contributions_by_claim: dict[str, list[dict[str, Any]]] = {
        claim_id: [] for claim_id in records
    }
    for path in sorted((root / "events").glob("JCG-E-*.yml")):
        event = yaml.safe_load(path.read_text(encoding="utf-8"))
        for claim_id in event.get("related_claims", []):
            if claim_id in events_by_claim:
                events_by_claim[claim_id].append(event)
    for path in sorted((root / "contributions").glob("JCG-C-*/metadata.yml")):
        contribution = yaml.safe_load(path.read_text(encoding="utf-8"))
        for claim_id in contribution.get("addresses_claims", []):
            if claim_id in contributions_by_claim:
                contributions_by_claim[claim_id].append(contribution)

    dependents: dict[str, list[str]] = {claim_id: [] for claim_id in records}
    for claim_id, record in records.items():
        dependency_ids = {
            dependency
            for item in record["evidence"]
            for dependency in item["uses_claim_ids"]
        }
        for dependency in dependency_ids:
            if dependency not in records:
                raise ValueError(f"{claim_id} depends on unknown public claim {dependency}")
            dependents[dependency].append(claim_id)
    for claim_ids in dependents.values():
        claim_ids.sort()
    return events_by_claim, contributions_by_claim, dependents


def claim_link(record: dict[str, Any]) -> str:
    return f"[{record['title']}]({record['slug']}.md)"
...
def render_page(
    ...
    lines = [
        ...
        f"**Record assessed on:** {record['assessment']['reviewed_on']}",
        "",
    ]
    if record["page"].get("context"):
        lines.extend(["## Definition and context", "", record["page"]["context"], ""])
    if record.get("scope"):
    ...
    if dependency_ids or dependent_ids:
        lines.extend(["## Claim relations", ""])
        if dependency_ids:
            lines.extend(["This claim uses:", ""])
            lines.extend(
                f"- {claim_link(records[claim_id])}" for claim_id in dependency_ids
            )
            lines.append("")
        if dependent_ids:
            lines.extend(["Claims that use this claim:", ""])
            lines.extend(
                f"- {claim_link(records[claim_id])}" for claim_id in dependent_ids
            )
            lines.append("")
    if events or contributions:
        lines.extend(["## In the chronological record", ""])
        for event in sorted(events, key=lambda item: (item["sort_date"], item["sequence"])):
            lines.append(
                f"- Event: [{event['title']}](../chronology-v2.md#event-{event['id'].lower()})"
            )
        for contribution in sorted(contributions, key=lambda item: item["sequence"]):
            lines.append(
                f"- Contribution: [{contribution['title']}](../chronology-v2.md#contribution-{contribution['id'].lower()})"
            )
        lines.append("")
    ...
    lines.extend(["", "[Back to the claim inventory](../claims-v3.md)", ""])
    return "\n".join(lines)


def expected_outputs(root: Path, records: dict[str, dict[str, Any]]) -> dict[Path, str]:
    events_by_claim, contributions_by_claim, dependents = load_record_relations(
        root, records
    )
    outputs = {root / "docs/claims-v3.md": render_index(records)}
    for record in records.values():
        claim_id = record["id"]
        outputs[root / "docs/claim-v3" / f"{record['slug']}.md"] = render_page(
            record,
            records,
            events_by_claim[claim_id],
            contributions_by_claim[claim_id],
            dependents[claim_id],
        )
    return outputs
...
def main() -> int:
    ...
    expected_pages = {path for path in outputs if path.parent == root / "docs/claim-v3"}
    actual_pages = set((root / "docs/claim-v3").glob("*.md")) if (root / "docs/claim-v3").is_dir() else set()
