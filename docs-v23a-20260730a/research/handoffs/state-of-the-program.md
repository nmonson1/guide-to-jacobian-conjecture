...
pages or papers. The [research index](../../research/index.md),
[stable claim catalogue](../../results/all-claims.md),
[working papers](../../research/papers.md), and [proof index](../../evidence/index.md)
...
**July 30 import checkpoints.** Two eleven-conversation intakes produced 13
and 29 scoped records; the newer covers six paper audits, degree-five/six,
Program 5 coupling, and Program 9 filtered actions. Its self-contained checks
replay, while Program 5 v3 and the reported Program 9 package have missing
inputs. Neither intake changes a paper or promotes a global theorem. Next,
extract atomic occurrences from every message and artifact, group exact
equivalents, type weaker relations, give each live class a home in Papers
1–6 or Paper 7 overflow, and verify the reverse paper-to-source map.
...
gluing, and the referenced Rees--Jordan checker remains unrecovered.
...
filtration with `delta(Q) <= 9`, giving `trdeg A_{<=6} <= 2` after
regenerating omitted intermediate bases. The residual frontier is unramified
`delta(Q) >= 10` plus a conceptual filtered-conormal, Wronskian, or
conductor theorem; this still is not full left-right minimality. A restricted
minimum over explicit frame moves is not enough: the target remains a
monotone under arbitrary polynomial left-right changes or a
non-degree-increasing canonical normalization.
The new companion/Jordan divisor budget is a useful prototype and Program 2
...
The new Lane 9 toolkit's six tests pass, but its upper-face matrices, actual
`F_2` blocks, and archived `C9` replay are absent. Its averaging lemma shows
that plain cyclic descent alone cannot obstruct the stable-support system.
Package unrecovered; complete-chain admissibility remains open.
[Exact Program 6 frontier](../../research/handoffs/plane-boundary-obstructions.md#4-the-live-frontier).
