...
This page is self-contained for choosing and specifying a task, not for
checking every proof. “Accepted as project input” authorizes reuse of the
exact cited statement at its stated scope. It does not mean externally
reviewed: no Program 1 claim below has independent specialist review
recorded. Open the version-pinned paper locator before depending on a proof
mechanism or hypothesis not reproduced here.
...
The claim graph marks items 1, the second half of item 6, and item 10 as
`recorded`; the remaining positive results are `proof offered`. Those labels
describe project evidence, not peer review.
