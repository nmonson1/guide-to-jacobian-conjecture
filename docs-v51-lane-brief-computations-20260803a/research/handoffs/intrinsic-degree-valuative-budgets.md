...
The operation audit now goes beyond uniform quadratic shears. For every
single pure monomial elementary source shear, the exact degree-at-most-six
image-algebra span is classified. The standard point has the pinned rank-81
minor; at zero shear parameter the low-degree span is generated by `1,Q,R`,
while at nonzero parameter it is generated by `1,sigma R` when the transformed
degree remains at most six and otherwise by constants. One-sided high-weight
words and mixed-sign separated reduced words also have transcendence degree
at most two in degree six. Unit: [`RMU-5DB60010`](../working-mathematics/units/RMU-5DB60010.md); proofs, operation conventions
and six exact checkers: [`research-notes/lane5-degree-budgets/`](lane-5-source-packet.md).
...
stabilization, and zero Wronskian is unknown when `delta(Q)>=10`. The new
word theorem does not cover arbitrary alternating or wild automorphisms.
...
filtered finite-dimensional subspace. Applying it to the particular
counterexample can now use the elementary and separated-word packet, but
still requires the missing full semidegree-normalization group. The
downstream target is:
...
Status: ready for the elementary and separated-word subgroup; blocked for the
full normalization group.

Inputs: [`RMU-5DB60010`](../working-mathematics/units/RMU-5DB60010.md) and [`research-notes/lane5-degree-budgets/`](lane-5-source-packet.md) for the
currently encoded source generators. A full-group conclusion additionally
requires the still-missing semidegree-normalization table.

Deliverable: compute `M` and the transported filtration for every encoded
generator and separated word, then identify the largest proved invariance
subgroup. State precisely which next alternating word pattern escapes the
separation theorem and either close it or give a concrete collision of
leading weights.
