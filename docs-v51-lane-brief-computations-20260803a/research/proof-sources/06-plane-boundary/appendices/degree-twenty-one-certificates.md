...
Published SHA-256: `dd35e507d5c0c41255853ff37676d96f2d09255f76ca9fe2f4e044fc99423ba2` · 25,157 bytes
...
<a id="label-thm-degree21-two-support-closure"></a>
- `thm:degree21-two-support-closure` — source line 378
<a id="label-cor-plane-degree-below-125"></a>
- `cor:plane-degree-below-125` — source line 464
<a id="label-subsec-two-chart-layer-seven"></a>
- `subsec:two-chart-layer-seven` — source line 484
<a id="label-prop-k4-chart-transition"></a>
- `prop:k4-chart-transition` — source line 493
<a id="label-thm-stored-terminal-layer-seven"></a>
- `thm:stored-terminal-layer-seven` — source line 548
...
\begin{theorem}[Direct closure of the two normalized \((8,28)\) supports]
\label{thm:degree21-two-support-closure}
After substitution of any of the five conjugate degree-\(21\) faces from
\cref{prop:degree21-belyi}, neither the truncated support
\[
\begin{aligned}
...
\end{aligned}
\]
nor the full support obtained by adjoining \((0,8)\) to \(N(P)\) and
\((0,12)\) to \(N(Q)\) has a vertex-saturated solution of
\([P,Q]=X^2\) over an algebraic closure of the coefficient field.
\end{theorem}

\begin{proof}[Exact computer-assisted proof]
The reconstruction begins with every lattice point in the two displayed
polygons and the deficiencies
\[
d_P(i,j)=j-2i+2,\qquad d_Q(i,j)=j-2i+3.
\]
Writing \(t=Y\), \(z=XY^2\), \(P=t^{-2}A(z,t)\), and
\(Q=t^{-3}B(z,t)\), direct differentiation turns the bracket equation into
\[
2AB_z-3A_zB+t(A_zB_t-A_tB_z)=z^2.
\]
Thus the order-\(r\) equation is triangular: its new coefficients occur
through a fixed linear map over
\(K_0=\mathbb Q[u]/(u^5-u^4+3u^3+3u^2+26)\), while its forcing term uses
only lower orders.  The replay forms these maps from the raw supports; it
does not read archived layer matrices or obstruction equations.

For the truncated support, the layer data are
\[
\begin{array}{c|rrrrr}
r&1&2&3&4&5\\ \hline
\dim U_r&19&21&13&0&0\\
\dim V_r&18&19&20&20&21\\
\operatorname{rank}\mathscr D_r&17&18&12&0&0\\
\dim\ker\mathscr D_r&2&3&1&0&0\\
\#\text{compatibilities}&0&0&7&18&0.
\end{array}
\]
The seven weight-three equations together with their products by the two
weight-one variables and the eighteen weight-four equations span all
fourteen weight-four monomials in the four effective variables.  An exact
\(14\)-by-\(14\) minor is nonzero.  Hence all four variables vanish on the
geometric obstruction locus, forcing both required top-vertex coefficients
to vanish, contrary to vertex saturation.

For the full support, the corresponding layer data through order eight are
\[
\begin{array}{c|rrrrrrrr}
r&1&2&3&4&5&6&7&8\\ \hline
\dim U_r&19&21&21&19&17&15&13&11\\
\dim V_r&18&19&20&20&21&20&19&18\\
\operatorname{rank}\mathscr D_r&17&18&18&18&17&15&13&11\\
\dim\ker\mathscr D_r&2&3&3&1&0&0&0&0\\
\#\text{compatibilities}&0&0&0&2&2&4&5&6.
\end{array}
\]
Up to a nonzero scalar, the unique layer-four equation is
\((t_{2,2}-\alpha t_{1,1}^2)^2\).  On its reduced support the two required
top vertices are nonzero constants times \(t_{1,1}^2\) and
\(t_{1,1}^3\).  The closed child \(t_{1,1}=0\) is therefore empty, and the
open child admits the exact weighted normalization \(t_{1,1}=1\),
\(t_{2,2}=\alpha\).  Continuing the raw recursion produces fifteen
equations in five variables, with \((1,3,5,6)\) distinct equations at
weights \((5,6,7,8)\).  The equations with zero-based indices
\(4,6,8,9,10,11\) are literally a subset of those fifteen.  Their zero
locus is empty by \cref{thm:terminal-toric-certificate}, so the full
fifteen-equation locus is empty as well.

Coefficients of deficiency greater than eight are only projected away: none
is divided by, and none can alter an equation of lower deficiency.  Hence
every full-support completion maps to this empty necessary-condition locus.
The square is retained scheme-theoretically; passing to its radical is used
only for point-set routing.  If the original finite-type obstruction algebra
were nonzero, its faithfully flat base change to an algebraic closure would
remain nonzero and have a maximal ideal, contradicting the established
geometric emptiness.  Thus the unreduced obstruction scheme is empty too.
The exact replay, stage hashes, denominator ledger, and the sole noncovering
adjacent-chart edge are recorded in
\path{research-notes/lane8-full-root-closure-20260803-v1/}.
\end{proof}

\begin{corollary}[Exclusion below maximum coordinate degree \(125\)]
\label{cor:plane-degree-below-125}
There is no characteristic-zero plane Keller counterexample satisfying
\(\max\{\deg P,\deg Q\}<125\).
\end{corollary}

\begin{proof}
Guccione--Guccione--Horruitiner--Valqui, Theorem~2.1, leave only the degree
pair \((72,108)\), up to exchanging coordinates, in this range
\cite{guccioneEtAl2022degree108}.  Their Proposition~4.1 and
Corollary~5.7 exclude the \((9,27)\) complete-chain case, while their
Proposition~4.3 routes the remaining \((8,28)\) case to precisely the two
normalized supports in \cref{thm:degree21-two-support-closure}.  That theorem
excludes both.

It is enough to work after extension to an algebraic closure.  If the
base-changed polynomial map is an isomorphism, its inverse is unique and
descends along the faithfully flat field extension.
\end{proof}

\subsection{A stored layer-five-through-seven certificate}
\label{subsec:two-chart-layer-seven}

The large layer-eight terminal certificate is not needed for the stored
degree-\(21\) transformed specialization.  A second exact calculation
obtains a contradiction using only its displayed layers five through seven.
The former identification of that transformed system with the layer-four
residual through a bare \(k=4\) wall shear is corrected below.

\begin{proposition}[Grading correction for the bare wall shear]
\label{prop:k4-chart-transition}
In the lower-face coordinates
\[
t=Y,\qquad z=XY^2,
\]
the bare wall shear \(Y'=Y+\lambda X^{-k}\) has exact form
\[
t'=t(1+h),\qquad z'=z(1+h)^2,qquad
h=\lambda t^{2k-1}z^{-k}.
\]
Its infinitesimal normal order is \(2k-1\).  In particular the bare
\(k=4\) shear begins at layer seven, not layer four.  No
filtration-preserving conjugacy with invertible associated graded can lower
that order.  Thus the recorded one-dimensional layer-four residual has not
been identified with this wall shear or with an actual adjacent
complete-chain chart.
\end{proposition}

\begin{proof}
Since \(X=z/t^2\), direct substitution gives
\[
Y'=t+\lambda(t^2/z)^k=t(1+h)
\]
and \(z'=X(Y')^2=z(1+h)^2\), proving the formula and its first normal order.
If \(\Phi\) preserves a filtration and induces an invertible map on the
associated graded, then the nonzero leading symbol of any filtered element
is carried to a nonzero symbol in the same degree.  Hence conjugation by
\(\Phi\) preserves the first nonzero normal order and cannot change seven
to four.
\end{proof}

Independently of this missing bridge, the archived transformed system uses
the adjacent blowdown coordinates
\[
...
\]
for a common linear form \(R\).  These equations define the system certified
next; the preceding proposition deliberately does not claim that every
full-support Newton branch reaches it.

\begin{theorem}[Stored transformed-system emptiness certificate]
\label{thm:stored-terminal-layer-seven}
The displayed layer-five-through-seven equations of the stored
degree-\(21\) transformed specialization have no common zero over the
...
system and has been replayed from the recovered source package.  It does not
prove that the one-dimensional layer-four residual reaches this system, that
the bare \(k=4\) shear is an admissible complete-chain operation, or that
every full-support branch is covered.  Those missing chart-provenance and
covering statements must be supplied before this certificate can close a
parent Newton-queue node.
...
The contribution of this project is the explicit degree-\(21\) boundary
description and an independently auditable direct reconstruction connecting
both normalized supports to exact terminal certificates.  The separate
stored adjacent-chart system remains unattached, but
\cref{thm:degree21-two-support-closure} does not use that proposed bridge.
~~~
