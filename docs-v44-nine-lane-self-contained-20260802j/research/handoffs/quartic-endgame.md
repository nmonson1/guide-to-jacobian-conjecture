...
<p class="claim-tag">Lane 4 · Updated 2 August 2026</p>
...
systems. The degree-five/six fixed-factor theorem additionally assumes
`V_P2(G,A,B)=empty`; its basepoint boundary is open.
...
Construct a rooted case tree beginning with an arbitrary normalized quartic
Keller map. Each edge must give its hypotheses, coordinate changes,
saturations, specializations, proof locator, and complementary boundary. A
leaf is accepted only when its theorem or certificate inputs match all
...
Inputs: the inventory above, linked retained units, and the complete source
tree rooted at [`manuscripts/02-low-degree/main.tex`](../proof-sources/02-low-degree/main.md).
...
Status: blocked on P2-L4A; local replay component.

Inputs: the terminal leaves actually used by the completed table.
...
- The degree-five/six fixed-factor theorem requires basepoint freeness.
