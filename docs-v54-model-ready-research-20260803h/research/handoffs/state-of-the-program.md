...
| --- | --- | --- | --- | --- |
| [1. Cubic normalization defects](cubic-flatness-normalization-defects.md) | A cubic Keller normalization can fail to be flat only on one exact finite defect carrier. | Ext, quadratic-resolvent, and Čech-saturation descriptions with a complete proof source. | Classify or exclude the closed-threefold three-torsion reflexive carrier. | **Ready:** local theorem/construction problem. |
| [2. Projective PRS boundary](boundary-completeness-torelli-at-infinity.md) | The known finite normalization must be extended across three fixed infinity charts before a Torelli statement is global. | Explicit generator pairs, a fixed multi-Rees compactification, and the finite \(T=0\) normalization. | Normalize and glue the three boundary charts, including the double-infinity corner. | **Ready:** all chart inputs are specified. |
