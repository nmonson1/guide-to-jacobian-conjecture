---
title: "Base Cover Monodromy And Deck Group"
description: "This current collection omits historical wording. Replaced records point to their current retained mathematics."
---

# Base Cover Monodromy And Deck Group

<p class="dek">This current collection omits historical wording. Replaced records point to their current retained mathematics.</p>

<span class="status status-kind">Result</span> <span class="status coverage-partial">Partial manuscript locator</span>

## Precise statement

Current mathematical statements are listed unit by unit below; superseded and archival wording is not reproduced.

## Claims in this result package

### [JCG-1B137277 · The function-field extension induced by the Alpöge map has degree 3 and…](../claims/JCG-1B137277.md)

The function-field extension induced by the Alpöge map has degree 3 and an S3 Galois closure, as described by the cited explicit cubic model.

*Defining · Primary Statement · Proof supplied*

### [JCG-D8D0210F · The displayed three-variable map has trivial rational deck group over its target:…](../claims/JCG-D8D0210F.md)

The displayed three-variable map has trivial rational deck group over its target: a rational self-map sigma satisfying F composed with sigma equals F must be the identity.

*Defining · Supporting Result · Proof supplied*

### [JCG-3C99797D · For the base discriminant complement, the fundamental group is identified with the…](../claims/JCG-3C99797D.md)

For the base discriminant complement, the fundamental group is identified with the parity fiber product {(n,sigma) in Z x S3 : parity(n)=sign(sigma)}.

*Defining · Supporting Result · Recorded*

### [JCG-45B208F0 · The map is prime under polynomial composition.](../claims/JCG-45B208F0.md)

The map is prime under polynomial composition.

*Shared · Supporting Result · Proof supplied*

### [JCG-504733CF · For every coprime normalized cubic-frame pair in the stated family, the inverse…](../claims/JCG-504733CF.md)

For every coprime normalized cubic-frame pair in the stated family, the inverse cubic is irreducible with nonsquare discriminant over C(a,b,c), so its generic Galois closure and geometric monodromy are S3.

*Shared · Supporting Result · Proof supplied*

### [JCG-8AA6A8C5 · The original three-variable map has the displayed invariant quotient cubic theta^3-2theta^2+B…](../claims/JCG-8AA6A8C5.md)

The original three-variable map has the displayed invariant quotient cubic theta^3-2theta^2+Btheta-2A, whose irreducibility and nonsquare discriminant give generic Galois group S3; the descendant ladder presents this same three-sheeted cover.

*Shared · Supporting Result · Recorded*

### [JCG-1D7EF048 · Replaced historical record](../claims/JCG-1D7EF048.md)

!!! warning "Replaced working statement"
    The earlier wording is suppressed. Continue with the current replacement mathematics below.

- [Corrected divisorial classification](../research/working-mathematics/units/RMU-C1DD871B.md): At the geometric generic point of every prime divisor \(H\subset Y\), exactly
one of the following types occurs:
\begin{enumerate}[label=\(U_\arabic*\),start=0]
\item all three sheets are unramified and retained;
\item all three sheets are unramified and exactly one is deleted;
\item all three sheets are unramified and exactly two are deleted;
\end{enumerate}
or
\begin{description}
\item[\(B\)] the inertia is a transposition, the ramified point is deleted,
and the unramified point is retained.
\end{description}
Outside \(S_F\) only \(U_0\) occurs, while at the generic point of a divisor
contained in \(S_F\) only \(U_1,U_2,B\) occur.  In particular, a deleted
three-cycle cannot occur.  The Galois closure of the generic cubic extension
has group \(S_3\).

*Shared · Generalization · Proof supplied*

## Evidence and manuscript boundary

Public sources are linked below. The linked working manuscript contains part of this grouped result, but it is not a complete source for every current defining claim.

Complete coverage requires an exact LaTeX location for every program-relevant defining claim. A locator records where the current statement and evidence boundary appear; it is not a proof-review status.

### Public sources

- [Galois structure of the new counterexample to the Jacobian conjecture: an explicit cubic model with S3 monodromy — is this known? — dorky](https://mathoverflow.net/questions/513387/)

## Credit

- Nathaniel Monson: research direction and accountable author; attributed by source
- Nathaniel Monson: research direction and mathematical responsibility; attributed by source
- dorky: derivation; documented authorship — Contribution recorded in Galois structure of the new counterexample.

## Connections

- [Base Map Fibers Image And Nonproperness](../collections/base-map-fibers-image-and-nonproperness.md)
- [The Common Cover of the Descendant Ladder](../collections/common-cover-of-the-descendant-ladder.md)
- [The Counterexample Is Prime Under Polynomial Composition](../collections/composition-primality.md)
- [Cubic Homogeneous Descendant Construction](../collections/cubic-homogeneous-descendant-construction.md)
- [Resolvent Restrictions on a Cubic Normalization Defect](../collections/cubic-resolvent-defect-exclusions.md)
- [Why the Double-Root Slice Is Affine Three-Space](../collections/double-root-affine-source.md)
- [Drużkowski Pairing](../collections/druzkowski-pairing.md)
- [A 38-Dimensional Hessian Quartic from Five Reduction Strategies](../collections/hessian-quartic-five-attacks.md)
- [Uniqueness in the Multiplication-Incidence Construction](../collections/multiplication-incidence-uniqueness.md)
- [The Root-Slope Mechanism and Cubic Cover Geometry](../collections/root-slope-and-cubic-cover-geometry.md)
- [Bounds and Obstructions for Symmetric Dilations](../collections/symmetric-dilation-bounds.md)

[Back to Results](../results/index.md)
