...
- `corrects` `JCG-8DD6ECB1`: The legacy statement omitted the target-basis hypotheses needed to turn the archived Macaulay minor into a unit-ideal certificate; the replacement states the exact conditional implication and preserves the old unit only as superseded history.
