...
known smooth-axis result is a model case; the desired theorem must explain
the singular height-two primes and the passage to the global Keller
boundary.
...
1. prove or sharply delimit a Keller-specific conormal-root/MCM pushdown;
2. derive any resulting exceptional-lattice inputs exactly;
3. keep finite flatness and recovery of the affine opening as separate
...
### [Four divisorial types](../units/RMU-C1DD871B.md)

`RMU-C1DD871B` · `proposition`

At the geometric generic point of every prime divisor of \(Y\), the retained
and deleted sheets have one of the four types \(U0\), \(U1\), \(U2\), or
\(B\).  Here \(U0\) has three retained unramified sheets; \(U1\) and \(U2\)
have respectively one and two deleted unramified sheets; and \(B\) has a
retained unramified sheet and a deleted ramified pair.  In particular a
deleted three-cycle cannot occur.  The generic monodromy of the cubic
extension is \(S_3\).
...
### [MCM criterion](../units/RMU-5D20CFBF.md)

`RMU-5D20CFBF` · `proposition`

If \(L\) is maximal Cohen--Macaulay over \(Q\), then \(B\) is flat over
\(A\).
...
### [Isolated-resolvent exclusion](../units/RMU-9D054DC3.md)

`RMU-9D054DC3` · `theorem`

If \(Q\) has an isolated singularity at a putative defect point, then the
cubic normalization is flat there.  More generally, restriction to the
height-two singular primes is injective on \(\Cl(Q)[3]\).
...
### [Smooth cubic-axis exclusion](../units/RMU-0DFDE6EE.md)

`RMU-0DFDE6EE` · `theorem`

Assume that the completed quadratic resolvent along its singular curve has a
one-parameter hypersurface model
\[
R=\C[[t,x_0,x_1,x_2]]/(\phi_t)
\]
whose central projective cubic is smooth, and that the transverse
three-torsion class is represented by a section of the relative
three-torsion group scheme of that smooth cubic family.  Then the associated
rank-one reflexive three-torsion module is MCM.  Hence this local model
cannot support a cubic normalization defect.
