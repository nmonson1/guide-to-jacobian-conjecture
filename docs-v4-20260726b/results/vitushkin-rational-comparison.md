...
**Credited to Terence Tao (derivation, exposition); Anonymous commenter 693534 (research assistance).**
...
The theorem-level package is centered on the following mechanism: Vitushkin's 1999 rational example has an analogous multiplication-map presentation and is generically two-to-one, but its source is a punctured plane, so the resulting map has a pole.  Its supporting records isolate the ingredients that establish the statement and the qualifications that control its scope.
