...
<p class="claim-tag">Lane 3 · Updated 2 August 2026</p>
...
A second exact family now isolates formal effectivity itself. Put

```text
A_alpha(c)=c(1+alpha*c),
B_(alpha,q)(c)=-2-4*alpha*c+q*alpha^2*c^2.
```

For `delta=q'-q`, a framed root translation of `c`-degree at most `D` exists
exactly when `delta*alpha^(D+2)=0`; it is unique and its residual is the single
term `(-1)^D delta alpha^(D+2)c^(D+2)`. Thus for `alpha=s mod s^M` its minimal
degree is `M-2`. Over `C[[s]]` every Artin truncation is compatibly framed
equivalent, while no polynomial framed equivalence exists. The stable
classification theorem says that the normalized generic fibres with
parameters `q` and `q'` are stably polynomially left--right equivalent if and
only if `q=q'` ([`RMU-9075E072`](../working-mathematics/units/RMU-9075E072.md)). Together with the effective
Nullstellensatz bound, this proves that the unrestricted stable-equivalence
complexity of the displayed family diverges. Unit: [`RMU-3FEF0011`](../working-mathematics/units/RMU-3FEF0011.md); complete
proof, dependency audit and three exact checkers:
[`research-notes/lane3-formal-effectivity/`](lane-3-source-packet.md).
...
Independently, sharpen the proved stable-effectivity result. Determine the
smallest possible complexity of an unrestricted stable polynomial
left--right equivalence modulo `s^M` between distinct `q`-fibres, or construct
new families in which the lower bound grows faster than the present linear
staircase. The theorem already separates the complete generic fibres, so this
problem concerns sharp quantitative complexity rather than missing
justification for non-effectivity.
...
### P3-L3D — Sharp stable-equivalence complexity

Status: ready.

Inputs: [`RMU-3FEF0011`](../working-mathematics/units/RMU-3FEF0011.md), [`RMU-9075E072`](../working-mathematics/units/RMU-9075E072.md),
`formal_effectivity_theorem.md`, its explicit source and target
automorphisms, and the dependency audit in
[`research-notes/lane3-formal-effectivity/`](lane-3-source-packet.md).

Deliverable: matching upper and lower bounds for the least complexity of an
unrestricted stable polynomial equivalence modulo `s^M`, or an explicit new
family with a stronger proved growth law. State the allowed source, target
and stabilization operations and the chosen complexity measure exactly.
