...
Choose distinct collision points represented by `v` and `v+u`, so `u!=0`
and `F(v+u)=F(v)`. The Keller condition implies that `u` and `v` are
automatically independent: if `v` were proportional to `u`, homogeneity would
give a nonzero eigenvalue of `JH(u)`, contradicting its nilpotence. Polarize
`H` to the symmetric trilinear tensor `mathcal T` and put
...
[`RMU-5C7E0011`](../working-mathematics/units/RMU-5C7E0011.md); theorem, original fifteen equations, reconstruction helper,
exact matrices and two successful checkers:
[`research-notes/lane7-split-incidence-20260802-v1/`](lane-7-source-packet.md).

There is one further geometric open condition. Put `w=C(a)A(a)u` and
`eta_ij=u_i*w_j-u_j*w_i`. The genuine projective marking incidence is the
union of the opens `eta_ij!=0` inside `P(ker M)`. Therefore a component of
`V(I_5(M))` contributes to collision geometry only if this Pluecker-open is
generically nonempty on it.
...
and determine whether `I_5(M)` has grade six. On every retained component,
also determine whether some `eta_ij` is nonzero at the generic point. The
first equality excludes corank two in the marking incidence; the second would
make the determinantal carrier a pure Cohen--Macaulay curve by
Eagon--Northcott; the Pluecker test decides whether that carrier contains
genuine marked collisions. None follows from the known smooth point or
expected codimension. A geometric interpretation of `M` compatible with the
`C*lambda+Sym^6(C^2)` action is preferred, but exact characteristic-zero
saturation, grade and generic-open certificates are also complete.
...
Inputs: [`RMU-5C7E0011`](../working-mathematics/units/RMU-5C7E0011.md) and the complete standalone packet in
...
explicit characteristic-zero point of the corank-two locus. For each
component retained from `I_5(M)`, certify that at least one `eta_ij` is
generically nonzero or discard it as a degenerate marking component. Preserve
the coefficient field and every saturation boundary.
...
matrix `M`, determinant `d`, and Pluecker opens from [`RMU-5C7E0011`](../working-mathematics/units/RMU-5C7E0011.md).
...
decomposition. Intersect the resulting components with the union of the
`D(eta_ij)` marking opens. Compare with the original fifteen-equation
saturation and preserve every intermediate ideal in a new versioned run.
...
- The determinantal carrier alone includes degenerate markings unless the
  Pluecker-open condition is retained.
