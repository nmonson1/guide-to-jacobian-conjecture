...
Published SHA-256: `2490b0a46364ea2d87601d0e0b169f5a425fd18566a2aa6da5511b3f94adad27` · 46,803 bytes
...
- `sec:span-two` — source line 589
<a id="label-lem-leading-image-dimension"></a>
- `lem:leading-image-dimension` — source line 596
<a id="label-lem-leading-image"></a>
- `lem:leading-image` — source line 625
<a id="label-cor-span-three-curve-entry"></a>
- `cor:span-three-curve-entry` — source line 688
<a id="label-prop-four-loci"></a>
- `prop:four-loci` — source line 739
<a id="label-rem-r-zero"></a>
- `rem:r-zero` — source line 815
<a id="label-sec-conic"></a>
- `sec:conic` — source line 840
<a id="label-eq-conic-form"></a>
- `eq:conic-form` — source line 844
<a id="label-lem-orbits"></a>
- `lem:orbits` — source line 850
<a id="label-lem-invariants"></a>
- `lem:invariants` — source line 890
<a id="label-eq-tab"></a>
- `eq:TAB` — source line 945
<a id="label-eq-chain"></a>
- `eq:chain` — source line 958
<a id="label-eq-divisibility"></a>
- `eq:divisibility` — source line 986
<a id="label-prop-terminal-conic"></a>
- `prop:terminal-conic` — source line 1013
<a id="label-sec-degree-seven-escape"></a>
- `sec:degree-seven-escape` — source line 1081
<a id="label-sec-computation"></a>
- `sec:computation` — source line 1105
<a id="label-sec-open-degree-four"></a>
- `sec:open-degree-four` — source line 1185
<a id="label-rem-leaf-accounting"></a>
- `rem:leaf-accounting` — source line 1194
...
The corrected Appelgate--Onishi plane theorem, in the per-coordinate form
proved by Nagata, therefore makes the plane map an automorphism
\cite{appelgateOnishi1985}\cite[Theorem~7.3]{nagata1988two}.  Its inverse over
...
\]
The plane pair is Keller and one coordinate has degree at most four, so the
same Appelgate--Onishi--Nagata theorem
\cite{appelgateOnishi1985}\cite[Theorem~7.3]{nagata1988two} makes it an
automorphism.
...
\label{lem:leading-image-dimension}
...
\[
[H_1:H_2:H_3]\colon\PP^2\dashrightarrow\PP^2
\]
...
\]
be the rational map defined by the leading forms.  Choose a nonconstant
rational function on \(C_4(F)\).  Its pullback is nonconstant on \(\PP^2\),
and its restriction to a sufficiently general line remains nonconstant.
Thus the restriction of \(\phi\) gives a nonconstant rational map
\(\PP^1\dashrightarrow C_4(F)\).  The curve is unirational, so its
normalization is \(\PP^1\) in characteristic zero.

Write the normalization map as
\[
\nu=[h_0:h_1:h_2]\colon\PP^1\longrightarrow C_4(F).
\]
Because \(\nu\) is finite birational and \(\deg C_4(F)=e\), the line bundle
\(\nu^*\mathcal O_{C_4(F)}(1)\) has degree \(e\).  Hence the \(h_i\) are
binary forms of degree \(e\) without a common zero.

The rational inverse of \(\nu\) on the generic point gives
\[
\psi=\nu^{-1}\circ\phi=[A:B]\colon\PP^2\dashrightarrow\PP^1,
\]
where \(A,B\) are coprime homogeneous forms of a common degree \(k\ge1\).
Put \(q_i=h_i(A,B)\).  Then
\[
[H_{4,1}:H_{4,2}:H_{4,3}]=[q_0:q_1:q_2].
\]
The forms \(q_i\) have no common irreducible divisor.  Indeed, at the generic
point of any prime divisor \(\Gamma\), the coprimality of \(A,B\) gives a
point \([A:B]\in\PP^1(k(\Gamma))\); if \(\Gamma\) divided all \(q_i\), the
basepoint-free triple \(h\) would vanish at that point after a field
extension.

Projective equality gives a common rational multiplier \(\lambda\) with
\(H_{4,i}=\lambda q_i\).  Write \(\lambda=u/v\) in lowest terms.  Polynomiality
of the \(H_{4,i}\) makes \(v\) divide every \(q_i\), so \(v\) is constant.
Thus \(\lambda=G\) is a homogeneous polynomial and
\[
H_4=G\,h(A,B),\qquad \deg G+ek=4.
\]
The four displayed numerical possibilities follow immediately.  When
\(e=1\), the image is a line, equivalently the leading target span is two.
\end{proof}

\begin{corollary}[The span-three entry to the curve tree]
\label{cor:span-three-curve-entry}
...
nondegenerate integral curve, so \cref{lem:leading-image} applies with
\(e\ge2\).
\end{corollary}

\begin{proof}
By \cref{lem:leading-image-dimension}, the image has dimension at most one.
It cannot be a point, since then the three coordinates are proportional, and
it cannot be a line, since a target line imposes a nonzero constant linear
relation among them.  Hence it is a nondegenerate curve.  The reduced image
closure of the irreducible variety \(\PP^2\) is integral.
\end{proof}

\begin{remark}
Relative algebraic closure is useful for detecting a primitive source
pencil, but it does not by itself prove that a chosen parametrization of the
image curve is birational.  For example,
\[
[x:y:z]\longmapsto[x^2:y^2:0]
\]
has line image and yields the degree-two parametrization \([s^2:1:0]\) after
adjoining \(s=x/y\).  The proof above avoids this shortcut by choosing the
normalization of the image first.
\end{remark}
...
common factor \(G\), every irreducible component of \(G\) being supported on
a special fiber of \(A/B\).
\end{enumerate}
...
\end{array}
\]
In a \(d=1\) case, \(s=A_0/B_0\) is a pencil of two independent linear forms,
and the reduced forms \(A,B\) are binary in those forms.  Let an irreducible
component \(\Gamma\) occur in \(G\) with multiplicity \(\mu\).  If \(s\) is
nonconstant on \(\Gamma\), then the reduced factor of \(Q\) and every nonzero
rational function of \(s\) are units at its generic point.  Valuation in
\[
R^4/Q^3=\sigma(s)
\]
gives
...
\(1\le\mu\le\deg G\le2\), a contradiction.  Hence every component of \(G\)
is a fiber line of \(s\), and \(G,P,Q\) are binary in the same two linear
forms.  The identity \(R^4=Q^3\sigma(s)\) puts \(R^4\) in their rational
function field.  Viewing \(R\) as a polynomial in a third independent linear
form, its leading coefficient cannot disappear on taking a fourth power;
therefore \(R\) is binary as well.  The only nonbinary composite case is
\((e,d)=(2,2)\) with \(G=1\).

In the composition-primitive coprime case \(w=\rho(t)\).  If a component of
multiplicity \(m\) lies in the corresponding quartic fiber, then
\[
4\nu(R)=c_\xi m,\qquad c_\xi\ge0,\qquad
\sum_{\xi\in\PP^1}c_\xi=3,
\]
with the denominator contribution incorporated at infinity.  Some
\(c_\xi\) is odd.  For that fiber \(4\mid c_\xi m\), so every component
multiplicity \(m\) is divisible by four.  The quartic fiber is therefore
\(\ell^4\).

Finally suppose \(e=1\) and \(G\ne1\).  If a component \(\Gamma\mid G\), of
multiplicity \(\mu\), carried a nonconstant ratio \(t=A/B\), then \(A,B\)
and \(\rho(t)\) would be units at its generic point.  The same identity would
give
\[
4\nu_\Gamma(R)=3\mu.
\]
But \(1\le\mu\le\deg G\le3\), which is impossible.  Thus \(t\) is constant
on every component of \(G\), so every such component is supported on a
special fiber.  Notice that the nonnegativity of \(c_\xi\) was used only
when \(G=1\); fixed components can cancel poles of \(\rho\).
\end{proof}
