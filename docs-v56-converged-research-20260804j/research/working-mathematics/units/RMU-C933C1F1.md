---
title: "For every \\(\\alpha,\\beta\\in\\C\\), the map \\(G_{\\alpha,\\beta}\\) is polynomial, \\[ \\det DG_{\\alp…"
description: "For every \\(\\alpha,\\beta\\in\\C\\), the map \\(G_{\\alpha,\\beta}\\) is polynomial,\n\\[\n\\det DG_{\\alpha,\\beta}=-2,\n\\]\nand its generic degree is three."
---

# For every \(\alpha,\beta\in\C\), the map \(G_{\alpha,\beta}\) is polynomial, \[ \det DG_{\alp…

`RMU-C933C1F1` · `proposition` · statement version `1`
