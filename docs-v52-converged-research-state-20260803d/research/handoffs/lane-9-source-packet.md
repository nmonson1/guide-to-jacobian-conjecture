...
- `research-notes/finite-diagnostics-20260803-v1/verify_lane9_wall_groupoid_packet.py` — `e71abf9aa41e4de9dbe78dd103a9c61a8b0c772bf45e3a37e0df18f19112aebc`
...
- `research-notes/lane9-wall-shear-20260802-v1/LANE9_CONTINUATION_V3_REPORT.md` — `c95708358a4bb8486d3823d15279e4acec3574fd55d2e5ad49234f032e57d510`
- `research-notes/lane9-wall-shear-20260802-v1/README.md` — `4c400dd7cec74320f4631e573ee8b1e87e4b5642f21b93976c18f496afca12a7`
- `research-notes/lane9-wall-shear-20260802-v1/research-notes/p6-chart-correspondence/F2_CYCLIC_WALL_DESCENT.md` — `2485be2f147247108a3c7dd828187a4985d5086d712ccc8ad14b2dd70696cce5`
- `research-notes/lane9-wall-shear-20260802-v1/research-notes/p6-chart-correspondence/WALL_SHEAR_DUAL_TRIPLE_OVERLAP.md` — `43a45be475ebeadb54f553f88038e446cce9a34d3e7d17e73ae92d48260da3b0`
- `research-notes/lane9-wall-shear-20260802-v1/research-notes/p6-chart-correspondence/WALL_SHEAR_GRADING_AUDIT.md` — `afeec62e2950cc4a120e9b5280dfbf04d1e1c86135a290c2bc3a1f0df84df745`
- `research-notes/lane9-wall-shear-20260802-v1/research-notes/p6-chart-correspondence/WALL_SHEAR_OVERLAP_THEOREM.md` — `94d278bd3285f8eadc2d31901a8518b84de8e8f53ec40d0d6cfa86975e412059`
- `research-notes/lane9-wall-shear-20260802-v1/research-notes/p6-chart-correspondence/WALL_SHEAR_REES_FLOW.md` — `9f763c0acd13a068b96cef1f073b46cf1a2afa9d82fa8fa32426c812648662ab`
- `research-notes/lane9-wall-shear-20260802-v1/research-notes/p6-chart-correspondence/degree21_k4_jet_transport_audit.py` — `4d240c1e6dc80412e1aa29fc55e7b6bf44208fb870577ab4f5ed642e7a758954`
- `research-notes/lane9-wall-shear-20260802-v1/research-notes/p6-chart-correspondence/degree21_k4_overlap_saturation.py` — `4a3c3d76037ebbae7328fe65a3e35d04ad85c3311fc72566881d5895b0511848`
- `research-notes/lane9-wall-shear-20260802-v1/research-notes/p6-chart-correspondence/degree21_k4_triple_overlap.py` — `5639081b7a0c4d72c8e4bf80170c9f43fcb45deb01146f323e1a396d65dd5988`
- `research-notes/lane9-wall-shear-20260802-v1/research-notes/p6-chart-correspondence/wall_shear_master_covariance.py` — `ae914d451e4ec3ab1b583c5f35c2e35c396c5596b9340dd817672961abc460cd`
- `research-notes/p6-chart-correspondence/LANE9_F2_PARAMETER_COMPLETE_RECURRENCE.md` — `3974cbe379942e4d4766d5e879d3a370adfde4eaea4936532f940a66253c3bb1`
- `research-notes/p6-chart-correspondence/lane9_f2_attachment_recurrence.py` — `f3c1719ae04ceb10f335b67d6aaa38bfa03c14186c2c8d32d9ba24262046563b`
...
<pre><code class="language-markdown">
# Lane 9 repair bundle

This bundle contains a proposed repair of the public Lane 9 handoff, a narrowed
version of claim `JCG-9667172F`, and twelve exact audit modules with seventy-three
regression tests.

## Main conclusions

1. The local quotient
   \(\ker D/\operatorname{im}\Theta\) is a tangent/moduli quotient, not an
   obstruction space; the obstruction space is \(\operatorname{coker}D\).
2. Complete-chain charts should be organized as a groupoid with exact
   coefficient, equation, operation, support, dual, forcing, inverse, and
   cocycle maps.
3. In the published lower-face coordinates `t=Y`, `z=XY^2`, the wall shear
   `Y -&gt; Y+lambda X^-k` has normal order `r=2k-1`.
4. Therefore the bare `k=4` shear first acts at normal layer seven and is the
   identity through layer six. The public layer-four/`k=4` identification
   requires a missing conjugation/reindexing or correction.
5. At layer seven the exact `k=4` tangent exits the old stored support at
   `A={-2,-1,4}` and `B={-2,-1}`.
6. On the full stored window, the exact first-order wall operator has 55
   internal entries, 97 coefficient-window exits, 31 terms above the cutoff,
   and three zero entries; its ambient transport is an exact exponential.
7. The maximal support-admissible layer-four polynomial field is Hamiltonian
   but not locally nilpotent, so it is not an additive algebraic shear. This
   is a concrete separation between infinitesimal support-admissibility and
   integrability in the complete-chain operation group.
8. Exact contragredient transport preserves every coefficient-residue
   pairing and every forcing/obstruction pairing on the wall-saturated spaces.
9. One pairwise wall overlap has deformation dimension 89, but the
   all-parameter triple-overlap core has dimension 68; 21 directions pass one
   overlap and fail the three-chart test.
10. Conjugating the layer-four candidate across the wall forces an explicit
    layer-eleven operation term outside the old window.
11. In the `F_2` gap-five quotient, the `k=4` wall parameter has `C_5`
    character four; scalar descent returns first at wall order five.
12. Assigning normal weight `-3` to that parameter aligns layer labels but
    leaves determinant-kernel defect `-3*z^-2`. The unique corrected vertical
    component is `g=-2*z^-4`.
13. The corrected field is the Laurent Hamiltonian lift of translation on the
    degree-eight quotient `H=2/(t*z)`, `Q=(z/t)^4`; its exact lift requires an
    eighth-root Kummer extension.
14. Since `Q` has normal pole order four, quotient translation has the disputed
    layer-four grading.  The strongest repair target is to compare the archived
    residual with the pullback of `16*partial_Q`, not with bare translation in
    `v=y`.

## Files to read first

- `LANE9_REPAIR_REPORT.md`
- `research-notes/p6-chart-correspondence/WALL_SHEAR_GRADING_AUDIT.md`
- `lane9-repair.patch`
- `PROPOSED_CROSS_FILE_EDITS.md`
...
```bash
git apply --check lane9-repair.patch
git apply lane9-repair.patch
python -m unittest discover \
  -s research-notes/p6-chart-correspondence \
  -p 'test_*.py' -v
```

The patch is prepared against the public Lane 9 and claim files on `main` and
adds the new research-note files. The broader portfolio and Program 6 edits are
listed separately because their generated release metadata should be refreshed
by the site pipeline.

## Repository actions

No commit, push, PR update, or merge was performed.


## Continuation v2

The bundle now also contains an exact ambient transported-window chart
groupoid:

- `wall_shear_master_covariance.py` proves and tests nonlinear master-equation
  covariance and the cumulative linearized chain map;
- `degree21_k4_overlap_saturation.py` constructs the minimal degree-21
  layer-15 wall saturation and computes old/transported intersections;
- `WALL_SHEAR_OVERLAP_THEOREM.md` records the theorem and its limitations;
- two additional test modules increase the exact regression count from
  20 to 32.

The new finite dimensions are:

```text
deformation: old 186, saturation 294, old∩transported 89, old+transported 283
equations:   old 257, saturation 300, old∩transported 216, old+transported 298
```

The base and opposite transported charts span both minimal saturations.


## Continuation v3

The bundle now adds six exact calculations:

- contragredient residue-dual and forcing transport for every wall-saturated
  basis vector;
- pairwise versus triple-overlap dimensions, including the 68-dimensional
  all-parameter deformation core;
- the layer-eleven commutator required when the layer-four candidate is
  transported across the wall;
- `C_5` character and Rees-weight bookkeeping for the `F_2` wall parameter;
- the exact failure of parameter weight alone and the unique corrected vertical term;
- conversion of that corrected term to a Laurent Hamiltonian flow requiring an eighth-root Kummer extension.

Key new numbers:

```text
E pairwise overlap 89; all-parameter core 68; pairwise-only 21
W pairwise overlap 216; all-parameter core 206; pairwise-only 10
all-parameter deformation constraints 118 = 97 first-order + 21 quadratic
```

The full suite now has 73 exact regression tests.


The v3 repair also proves that normal weight `-3` alone does not turn the bare
`k=4` wall into a layer-four kernel field.  The exact defect is `-3*z^-2`;
the unique vertical correction is `g=-2*z^-4`.


The corrected field is linearized by the degree-eight monomial quotient
`H=2/(t*z)`, `Q=(z/t)^4`, where it becomes `Q -&gt; Q+16*s`.  The eighth-root
Kummer extension is exactly the inverse of this lattice-index-eight quotient.


In the stored adjacent blowdown variables `u=(x*y)^(-1)`, `v=y`, the quotient
field is exactly `K(H,Q)=K(u,v^8)`.  The corrected field descends to this
`mu_8` quotient; the bare `k=4` wall is translation in `v` and is a different
operation.


Because `Q=(z/t)^4` has normal pole order four, the quotient translation has
the disputed layer-four grading.  The strongest candidate repair is now to
match the archived residual to the pullback of `16*partial_Q`, rather than to
the bare layer-seven translation in `v=y`.
</code></pre>
