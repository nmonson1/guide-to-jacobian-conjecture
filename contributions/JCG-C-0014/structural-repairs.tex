...
\begin{itemize}
\item coprime homogeneous forms \(A,B\in k[x,y,z]_k\) of a common degree
  \(k\geq 1\);
...
rational generator of \(k(C)\subset k(\mathbf P^2)\) to a sufficiently
...
degree \(k\).  Since \(C\) is a curve, \(\psi\) is nonconstant and \(k\geq1\).
Put
\[
