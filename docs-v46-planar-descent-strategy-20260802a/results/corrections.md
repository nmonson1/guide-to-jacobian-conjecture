---
title: "Corrections and scope changes"
description: "Tagged records that correct, narrow, or explicitly retire an earlier line of argument."
---

# Corrections and scope changes

<p class="dek">Corrections live in the same claim graph as results. They are not buried in release notes.</p>

## [JCG-0632B2B1 · On the five explicit degree-eight coordinate directions, the corrected quadratic/cubic equations lift precisely along the pure source-shear or pure target-shear sectors; a residual four-dimensional tangent survives to order three in a larger complement, and the full degree-four modular calculation remains exploratory.](../claims/JCG-0632B2B1.md)

On the five explicit degree-eight coordinate directions, the corrected quadratic/cubic equations lift precisely along the pure source-shear or pure target-shear sectors; a residual four-dimensional tangent survives to order three in a larger complement, and the full degree-four modular calculation remains exploratory.

## [JCG-099F60DA · The initial theorem claiming all finite quadratic-parameter incidence surfaces are of log general type was overbroad because ramification over parameter infinity overlaps the reduced boundary. The retained theorem requires squarefree behavior at infinity or the corrected log-pair hypotheses.](../claims/JCG-099F60DA.md)

The initial theorem claiming all finite quadratic-parameter incidence surfaces are of log general type was overbroad because ramification over parameter infinity overlaps the reduced boundary. The retained theorem requires squarefree behavior at infinity or the corrected log-pair hypotheses.

## [JCG-138BEDA2 · Open Conjecture: Keller hypotheses force the cubic normalization defect to vanish.](../claims/JCG-138BEDA2.md)

Keller hypotheses force the cubic normalization defect to vanish.

**Current replacement:** [RMU-34FBCB8D](../research/working-mathematics/units/RMU-34FBCB8D.md)

Does the Keller condition force the finite cubic normalization
\(\overline X\to\A^3\) to be flat?

## [JCG-1D7EF048 · Cubic Keller Maps Have Only the U1, U2, and B Divisorial Types](../claims/JCG-1D7EF048.md)

For a complex generic-degree-three Keller map, every target prime divisor has a retained unramified sheet and hence only the U1, U2, or B deleted-sheet type; a deleted three-cycle is impossible, and the generic monodromy is S3.

**Current replacement:** [RMU-C1DD871B](../research/working-mathematics/units/RMU-C1DD871B.md)

At the geometric generic point of every prime divisor \(H\subset Y\), exactly
one of the following types occurs:
\begin{enumerate}[label=\(U_\arabic*\),start=0]
\item all three sheets are unramified and retained;
\item all three sheets are unramified and exactly one is deleted;
\item all three sheets are unramified and exactly two are deleted;
\end{enumerate}
or
\begin{description}
\item[\(B\)] the inertia is a transposition, the ramified point is deleted,
and the unramified point is retained.
\end{description}
Outside \(S_F\) only \(U_0\) occurs, while at the generic point of a divisor
contained in \(S_F\) only \(U_1,U_2,B\) occur.  In particular, a deleted
three-cycle cannot occur.  The Galois closure of the generic cubic extension
has group \(S_3\).

## [JCG-1E683D42 · The upper face has common approximate root R=y^7(y-1); the exact layer cokernels persist, and the first nonlinear compatibility condition forces the first correction to be tangent to the locus (R^2,R^3), rather than yielding a contradiction.](../claims/JCG-1E683D42.md)

The upper face has common approximate root R=y^7(y-1); the exact layer cokernels persist, and the first nonlinear compatibility condition forces the first correction to be tangent to the locus (R^2,R^3), rather than yielding a contradiction.

## [JCG-248AD7B9 · Flatness of the finite cubic normalization is equivalent to vanishing of a finite defect module expressible through Ext data.](../claims/JCG-248AD7B9.md)

Flatness of the finite cubic normalization is equivalent to vanishing of a finite defect module expressible through Ext data.

**Current replacement:** [RMU-1A8D0001](../research/working-mathematics/units/RMU-1A8D0001.md)

Let R=C[y_1,y_2,y_3], let B be the finite normalization attached to a generic-degree-three Keller map, and write B=R direct-sum E by trace. Then B and E are reflexive and Delta_F=Ext^1_R(B,R)=Ext^1_R(E,R) has finite length. Its support is exactly the locus where B is not free over R. Consequently B is finite flat over R if and only if Delta_F=0. At a closed point y, a minimal resolution 0 -> A^b -> A^(b+2) -> E_y -> 0 over A=R_y presents (Delta_F)_y as the cokernel of the dual matrix, with b minimal generators.

## [JCG-39A7571B · Corrected Valuation Identity for Primitive Quartic Fixed Components](../claims/JCG-39A7571B.md)

In the primitive fixed-component branch P=GA, Q=GB with gcd(A,B)=1 and R=(H3)_3, write t=A/B and R^4/Q^3=rho(t). If an irreducible component Gamma occurs in G with multiplicity s and in the reduced special fiber A-xi B with multiplicity m, define c_xi=ord_xi(rho) for finite xi and c_infinity=ord_infinity(rho)+3. Then 4 nu_Gamma(R)=3s+c_xi m and the coefficients satisfy sum c_xi=3.

## [JCG-427C7334 · The vertex-saturated truncated Newton support is empty in characteristic zero for every one of the five lower faces.](../claims/JCG-427C7334.md)

The vertex-saturated truncated Newton support is empty in characteristic zero for every one of the five lower faces.

**Current replacement:** [RMU-6D8E0010](../research/working-mathematics/units/RMU-6D8E0010.md)

For the pinned truncated Newton supports, the supplied field relations reconstruct the exact fifteen quintic face equations and their open factors. The fourteen displayed coefficient vectors span the required truncated-root obstruction space: an exact fourteen-by-fourteen minor is nonzero (and equals 894 modulo 2053 in the compact certificate). Consequently the pinned vertex-saturated truncated-root system has no solution in characteristic zero under exactly the stored normalizations and open-locus hypotheses.

## [JCG-48D499D2 · Genuinely Nonbinary Primitive Fixed Components Are Closed](../claims/JCG-48D499D2.md)

Assume the Program 2 four-loci reduction and the standard polynomial-centralizer theorem used in the proof note. Every genuinely nonbinary primitive fixed-component branch is closed: a fixed factor of degree two is eliminated by the corrected valuation identity or enters the existing same-fiber rank-one chart, while a fixed factor of degree one is aligned-automorphic, binary, or incompatible with the Keller condition.

## [JCG-69028659 · The Regular One-Divisor Plane-Cubic Axis Resolvent Model Is Flat](../claims/JCG-69028659.md)

Let a complete normal Cohen-Macaulay threefold resolvent germ admit a proper birational regular model with one exceptional prime Cartier divisor over a smooth curve and a regular transverse surface slice. If the scheme-theoretic exceptional fiber is a plane cubic C with O_C(-E)=O_C(1), then every rank-one reflexive sheaf L with L^[3]=O is maximal Cohen-Macaulay; under the cubic-resolvent reduction, the associated cubic normalization is flat.

**Current replacement:** [RMU-A4B084FF](../research/working-mathematics/units/RMU-A4B084FF.md)

In the quadratic-resolvent setup above, suppose there exists a rank-one
reflexive \(Q\)-module \(M\) such that
\begin{enumerate}[label=(\roman*)]
\item \(M^{[3]}\simeq Q\);
\item \(M\) is maximal Cohen--Macaulay; and
\item for every height-two singular prime \(\mathfrak p\), the classes of
\(M_{\mathfrak p}\) and \(L_{\mathfrak p}\) agree in
\(\Cl(Q_{\mathfrak p})\).
\end{enumerate}
Then \(L\simeq M\), and the cubic normalization \(B\) is finite flat over
\(A\).

## [JCG-79BDB7F6 · A Cubic Defect Forces a Nonisolated Quadratic-Resolvent Singularity](../claims/JCG-79BDB7F6.md)

For the quadratic resolvent Q of a complex cubic Keller normalization, an isolated singularity cannot support a nonflat cubic defect; moreover, three-torsion in Cl(Q) is detected at the height-two singular primes.

**Current replacement:** [RMU-13177F24](../research/working-mathematics/units/RMU-13177F24.md)

Let \((Q,\mathfrak n)\) be a three-dimensional normal local hypersurface over
\(\C\), and put \(U=\Spec Q\setminus\{\mathfrak n\}\).
\begin{enumerate}[label=(\roman*)]
\item If \(Q\) has an isolated singularity, then \(\Cl(Q)[3]=0\).
\item In general, restriction induces an injection
\[
\Cl(Q)[3]\hookrightarrow
\bigoplus_{\substack{\mathfrak p\in\Sing(Q)\\\operatorname{ht}\mathfrak p=2}}
\Cl(Q_{\mathfrak p})[3].
\]
\end{enumerate}

## [JCG-82044B17 · Technical claim](../claims/JCG-82044B17.md)

The corrected cubic-pencil reduction says that dP wedge dQ wedge dh=0 forces either a nontrivial fixed component/gcd, a pencil-cube case, or dependence on two linear forms.

## [JCG-89916A56 · Open Question: Does the U1/U2/B trichotomy hold in the claimed generality beyond the normalized family?](../claims/JCG-89916A56.md)

Does the U1/U2/B trichotomy hold in the claimed generality beyond the normalized family?

**Current replacement:** [RMU-C1DD871B](../research/working-mathematics/units/RMU-C1DD871B.md)

At the geometric generic point of every prime divisor \(H\subset Y\), exactly
one of the following types occurs:
\begin{enumerate}[label=\(U_\arabic*\),start=0]
\item all three sheets are unramified and retained;
\item all three sheets are unramified and exactly one is deleted;
\item all three sheets are unramified and exactly two are deleted;
\end{enumerate}
or
\begin{description}
\item[\(B\)] the inertia is a transposition, the ramified point is deleted,
and the unramified point is retained.
\end{description}
Outside \(S_F\) only \(U_0\) occurs, while at the generic point of a divisor
contained in \(S_F\) only \(U_1,U_2,B\) occur.  In particular, a deleted
three-cycle cannot occur.  The Galois closure of the generic cubic extension
has group \(S_3\).

## [JCG-8DD6ECB1 · The six selected full-support obstruction equations generate the unit ideal over the quintic field, certified by a specified 7121-by-7121 exact minor whose reduction has determinant 859 modulo 2053.](../claims/JCG-8DD6ECB1.md)

The six selected full-support obstruction equations generate the unit ideal over the quintic field, certified by a specified 7121-by-7121 exact minor whose reduction has determinant 859 modulo 2053.

**Current replacement:** [RMU-ED13585F](../research/working-mathematics/units/RMU-ED13585F.md)

Suppose replay of the archived Macaulay data confirms that its \(7{,}121\)
rows are the complete ordered target monomial basis, that this basis contains
\(1\), and that the certified \(7{,}121\)-by-\(7{,}121\) minor uses every
target row.  Then, for the stored normalized full-support system, the six
selected obstruction equations generate the unit ideal over \(K_0\).

## [JCG-9667172F · The Missing Layer-Four Operation Is the k=4 Newton-Chart Transition](../claims/JCG-9667172F.md)

For the stored degree-21 terminal face, the one-dimensional layer-four residual kernel is not realized by a fixed-chart algebraic operation; it is the k=4 complete-chain rechart, after which the adjacent-chart Jacobian equations force L=0, F5=0, and a common approximate root P2=C R^2, Q3=G R^3.

**Current replacement:** [RMU-6C9E0010](../research/working-mathematics/units/RMU-6C9E0010.md)

In lower-face coordinates t=Y, z=XY^2, the wall shear Y'=Y+lambda X^(-k) has t'=t(1+h), z'=z(1+h)^2 with h=lambda t^(2k-1)z^(-k), so its infinitesimal normal order is r=2k-1. For k=4 the bare wall therefore begins at layer seven, not four, and no filtration-preserving conjugacy with invertible associated graded lowers it to layer four. Through layer 15, the exact density transports form an additive finite wall groupoid with nonlinear master-equation covariance, inverse and cocycle laws, contragredient residue-dual transport, and explicit pairwise/triple intersections. The corrected associated-graded layer-four candidate is a Hamiltonian flow that linearizes as Q -> Q+16s on a degree-eight monomial quotient, but its identification with the archived layer-four residual and the intended adjacent complete-chain chart remains open.

## [JCG-978BC8D0 · The equivariant cubic rank-six compression condition has a 20-dimensional affine slice, and the quartic obstruction functional is identically one on that slice; modulo two target-row directions this is the natural transverse compression space.](../claims/JCG-978BC8D0.md)

The equivariant cubic rank-six compression condition has a 20-dimensional affine slice, and the quartic obstruction functional is identically one on that slice; modulo two target-row directions this is the natural transverse compression space.

**Stronger current result:** [RMU-5A110520](../research/working-mathematics/units/RMU-5A110520.md)

For the displayed Program 5 eleven-variable tensors Q and C at P0=-d^2 e_a, the space of weight-preserving quadratic source fields has dimension 115. The affine equations killing the a,d,q,h,k cubic rows have rank 95 and direction space K_row of dimension 20. The tangent space K_rank to the rank-at-most-six cubic-coordinate locus has dimension 22, and K_row is contained in K_rank. On the full affine plane P0+K_rank, the exact quartic functional Lambda_4(O_4(P)) is identically 1.

## [JCG-A5938472 · For a finite normalization of the cubic cover, the trace-zero module is reflexive and the nonflat locus is finite; flatness and the global cubic package require an additional Cohen-Macaulay hypothesis.](../claims/JCG-A5938472.md)

For a finite normalization of the cubic cover, the trace-zero module is reflexive and the nonflat locus is finite; flatness and the global cubic package require an additional Cohen-Macaulay hypothesis.

**Current replacement:** [RMU-1A8D0001](../research/working-mathematics/units/RMU-1A8D0001.md)

Let R=C[y_1,y_2,y_3], let B be the finite normalization attached to a generic-degree-three Keller map, and write B=R direct-sum E by trace. Then B and E are reflexive and Delta_F=Ext^1_R(B,R)=Ext^1_R(E,R) has finite length. Its support is exactly the locus where B is not free over R. Consequently B is finite flat over R if and only if Delta_F=0. At a closed point y, a minimal resolution 0 -> A^b -> A^(b+2) -> E_y -> 0 over A=R_y presents (Delta_F)_y as the cokernel of the dual matrix, with b minimal generators.

## [JCG-AB57A9B2 · Equivalent Reformulation: The defect problem admits a resolvent maximal-Cohen–Macaulay reformulation.](../claims/JCG-AB57A9B2.md)

The defect problem admits a resolvent maximal-Cohen–Macaulay reformulation.

**Current replacement:** [RMU-1A8D0004](../research/working-mathematics/units/RMU-1A8D0004.md)

Let T be the normalization in the S_3 Galois closure, Q=T^(A_3), and L the nontrivial cubic eigensheaf. Then Q is a normal finite-flat quadratic R-algebra, T=Q direct-sum L direct-sum L^[2], L^[3]=Q, and the quadratic involution satisfies sigma^*L=L^[2]=L^dual. As an R-module, the cubic trace-zero summand E is isomorphic to L. Hence Delta_F=Ext^1_R(L,R), and B is finite flat over R if and only if L is maximal Cohen-Macaulay over Q.

## [JCG-BD8A0F77 · For every generic-degree-three Keller normalization, base change to the affine source splits the cubic algebra as S times S[eta]/(eta^2-D), implying flatness at every attained target point and a canonical marked-root form on the source.](../claims/JCG-BD8A0F77.md)

For every generic-degree-three Keller normalization, base change to the affine source splits the cubic algebra as S times S[eta]/(eta^2-D), implying flatness at every attained target point and a canonical marked-root form on the source.

**Current replacement:** [RMU-1A8D0003](../research/working-mathematics/units/RMU-1A8D0003.md)

For every generic-degree-three complex Keller normalization, base change to the affine source gives B tensor_R S isomorphic to S times C, where the S-factor is the canonical marked section and C is a normal quadratic S-algebra. After choosing a generator eta of its trace-zero summand, C is isomorphic to S[eta]/(eta^2-D). The generator and D are choice-dependent. The normalization is free over every attained target value, and Supp(Delta_F) is contained in the omitted-value locus, hence in the singular locus of the reduced nonproperness set.

## [JCG-C04524CF · Every generic-degree-three Keller map factors through a finite normal cubic cover whose trace-zero module is rank-two reflexive and locally free away from finitely many target points.](../claims/JCG-C04524CF.md)

Every generic-degree-three Keller map factors through a finite normal cubic cover whose trace-zero module is rank-two reflexive and locally free away from finitely many target points.

**Stronger current result:** [RMU-1A8D0001](../research/working-mathematics/units/RMU-1A8D0001.md)

Let R=C[y_1,y_2,y_3], let B be the finite normalization attached to a generic-degree-three Keller map, and write B=R direct-sum E by trace. Then B and E are reflexive and Delta_F=Ext^1_R(B,R)=Ext^1_R(E,R) has finite length. Its support is exactly the locus where B is not free over R. Consequently B is finite flat over R if and only if Delta_F=0. At a closed point y, a minimal resolution 0 -> A^b -> A^(b+2) -> E_y -> 0 over A=R_y presents (Delta_F)_y as the cokernel of the dual matrix, with b minimal generators.

## [JCG-DCDBDDDB · Partial Defect Exclusion Theorem: A minimal defect can only occur over an omitted singleton fiber of scheme length at least four.](../claims/JCG-DCDBDDDB.md)

A minimal defect can only occur over an omitted singleton fiber of scheme length at least four.

**Current replacement:** [RMU-1A8D0005](../research/working-mathematics/units/RMU-1A8D0005.md)

At a point y in Supp(Delta_F), completion of the cubic normalization has one normal local factor of rank three. Its cubic fraction-field extension is non-Galois and therefore has S_3 Galois closure. If b is the presentation number from RMU-1A8D0001, then the finite normalization fibre is supported at one point and has scheme length b+3, hence at least four; length four is exactly the one-generator stratum.
