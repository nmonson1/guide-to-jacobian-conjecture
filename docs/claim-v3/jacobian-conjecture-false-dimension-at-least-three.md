---
title: "Failure of the Jacobian conjecture in dimensions at least three"
description: "The Jacobian conjecture is false in every dimension n at least 3."
---

# Failure of the Jacobian conjecture in dimensions at least three

The Jacobian conjecture is false in every dimension n at least 3.

**Status:** Proof offered — review pending<br>
**Primary source:** [A Counterexample to the Jacobian Conjecture](https://www.ulam.ai/research/jacobian.pdf)<br>
**Record assessed on:** 2026-07-22

## Scope

The complex Jacobian conjecture in every finite dimension n at least 3.

## Claim relations

This claim uses:

- [Constant Jacobian determinant of the Alpöge map](alpoge-map-constant-jacobian.md)
- [An explicit triple collision](alpoge-map-triple-collision.md)

Claims that use this claim:

- [The Jacobian conjecture](the-jacobian-conjecture.md)
- [Failure of the all-dimensional Vanishing Conjecture](all-dimensional-vanishing-conjecture-false.md)
- [Failure of the Dixmier conjecture in dimensions at least three](dixmier-conjecture-false-dimension-at-least-three.md)

## In the chronological record

- Event: [Alpöge posts the explicit three-variable counterexample](../chronology-v2.md#event-jcg-e-0007)
- Event: [Formal Conjectures PR 4474 opens](../chronology-v2.md#event-jcg-e-0009)
- Event: [A second Formal Conjectures pull request is closed unmerged](../chronology-v2.md#event-jcg-e-0017)
- Contribution: [A Counterexample to the Jacobian Conjecture](../chronology-v2.md#contribution-jcg-c-0002)
- Contribution: [Formal Conjectures PR 4474 — Jacobian disproof](../chronology-v2.md#contribution-jcg-c-0004)
- Contribution: [Formal Conjectures PR 4486 — sorry-free refutation over Q](../chronology-v2.md#contribution-jcg-c-0012)

## Credit

Levent Alpöge, Akhil Mathew, Paul Lezeau; AI assistance: Fable; +1 more

<details>
<summary>Full credit and attribution basis</summary>

- **Akhil Mathew** — problem suggestion; credited by the source. problem suggestion as reported by the source
- **Fable** (AI system) — research assistance; credited by the source. work leading to the example as reported by the source
- **Levent Alpöge** — discovery, construction; credited by the source. formula announced by the named person
- **Paul Lezeau** — formalization; documented authorship. Contribution recorded in Formal Conjectures PR 4474 — Jacobian disproof.
- **techno-optimist** — formalization; documented authorship. Contribution recorded in Formal Conjectures PR 4486 — sorry-free refutation over Q.

</details>

## Evidence

### Counterexample

**Full proof offered.** Proof by counterexample and stabilization.

- [Proof by counterexample and stabilization](https://www.ulam.ai/research/jacobian.pdf)

### Lean formalization

**Full proof offered.** Lean formalization.

- [Lean formalization](https://github.com/google-deepmind/formal-conjectures/pull/4474)

### Lean formalization

**Full proof offered.** Independent lean formalization.

- [Independent lean formalization](https://github.com/google-deepmind/formal-conjectures/pull/4486)

## Review record

This guide has not yet recorded an independent expert or machine check covering the whole claim.

## Limitations

- Failure of the conjecture in dimension 2.

## Sources

- [A Counterexample to the Jacobian Conjecture](https://www.ulam.ai/research/jacobian.pdf) — primary
- [Hello there the Jacobian conjecture is false](https://x.com/__alpoge__/status/2079028340955197566)
- [feat: add Jacobian disproof](https://github.com/google-deepmind/formal-conjectures/pull/4474)
- [feat(JacobianConjecture): sorry-free refutation at n = 3 over Q](https://github.com/google-deepmind/formal-conjectures/pull/4486)

[Back to the claim inventory](../claims-v3.md)
