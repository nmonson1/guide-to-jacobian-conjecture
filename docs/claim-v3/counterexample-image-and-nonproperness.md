---
title: "Image and nonproperness set of the counterexample"
description: "The image of the Alpöge map is complex affine 3-space minus the triple-root curve Gamma, while its nonproperness set is the discriminant hypersurface V(Delta)."
---

# Image and nonproperness set of the counterexample

The image of the Alpöge map is complex affine 3-space minus the triple-root curve Gamma, while its nonproperness set is the discriminant hypersurface V(Delta).

**Status:** Proof offered — review pending<br>
**Primary source:** [A Counterexample to the Jacobian Conjecture](https://www.ulam.ai/research/jacobian.pdf)<br>
**Record assessed on:** 2026-07-22

## Scope

The set-theoretic image and nonproperness locus of the displayed Alpöge map over the complex numbers.

## Claim relations

This claim uses:

- [Generic degree and fiber stratification of the counterexample](counterexample-fiber-stratification.md)
- [The double-root resultant slice is affine three-space](double-root-resultant-slice-affine-three-space.md)

## In the chronological record

- Event: [A technical note gives exact checks and the binary-cubic model](../chronology-v2.md#event-jcg-e-0008)
- Event: [Public working seminars analyze the geometry and function field](../chronology-v2.md#event-jcg-e-0011)
- Contribution: [A Counterexample to the Jacobian Conjecture](../chronology-v2.md#contribution-jcg-c-0002)

## Credit

Alejandro Radisic

<details>
<summary>Full credit and attribution basis</summary>

- **Alejandro Radisic** — formalization; credited by the source. author and maintainer of the pinned Lean development

</details>

## Evidence

### Proof

**Full proof offered.** Proof and exact formulas.

- [Proof and exact formulas](https://www.ulam.ai/research/jacobian.pdf)

### Lean formalization

**Partial support.** The pinned README identifies the relevant Lean theorems and reports no `sorry` and no axioms beyond Mathlib. No fresh `lake build` was run for this guide.

What this evidence does not settle:

- The same README expressly excludes S3 monodromy/irreducibility, the properness identification, scheme-level factorization, the cohomological torsor proof, and analytic derivative identification.

## Review record

This guide has not yet recorded an independent expert or machine check covering the whole claim.

## Limitations

- That omitted loci and nonproperness sets coincide for Keller maps.
- A classification of nonproperness sets.

## Sources

- [alpoge-lean v0.4.0 documentation](https://github.com/alerad/alpoge-lean/blob/95897469d48ba97f1a80a2db6553fd2f0f43834b/README.md)
- [alpoge-lean v0.4.0](https://github.com/alerad/alpoge-lean/releases/tag/v0.4.0)
- [A Counterexample to the Jacobian Conjecture](https://www.ulam.ai/research/jacobian.pdf) — primary
- [Hello there the Jacobian conjecture is false](https://x.com/__alpoge__/status/2079028340955197566)

[Back to the claim inventory](../claims-v3.md)
