---
title: "Three-dimensional Keller maps of every generic degree"
description: "For every integer d at least 3, there is a nonproper Keller map from complex affine 3-space to itself with generic degree d."
hide:
  - navigation
search:
  exclude: true
robots: noindex, nofollow
---

# Three-dimensional Keller maps of every generic degree

For every integer d at least 3, there is a nonproper Keller map from complex affine 3-space to itself with generic degree d.

**Status:** Proof offered — review pending<br>
**Primary source:** [A Counterexample to the Jacobian Conjecture](https://www.ulam.ai/research/jacobian.pdf)<br>
**Record assessed on:** 2026-07-22

## Scope

For every integer d at least 3, polynomial Keller self-maps of complex affine 3-space having generic degree d.

## In the chronological record

- Event: [A technical note gives exact checks and the binary-cubic model](../chronology-v2.md#event-jcg-e-0008)
- Event: [Uniform fixed-dimension families of every generic degree are posted](../chronology-v2.md#event-jcg-e-0012)
- Contribution: [A Counterexample to the Jacobian Conjecture](../chronology-v2.md#contribution-jcg-c-0002)
- Contribution: [Weighted lifts from the Jacobian counterexample](../chronology-v2.md#contribution-jcg-c-0007)

## Credit

algal

<details>
<summary>Full credit and attribution basis</summary>

- **algal** — construction, computation; documented authorship. Contribution recorded in Weighted lifts from the Jacobian counterexample.

</details>

## Evidence

### Proof

**Full proof offered.** Explicit family construction.

- [Explicit family construction](https://www.ulam.ai/research/jacobian.pdf)

### Proof

**Full proof offered.** Independent uniform construction and exact atlas.

- [Independent uniform construction and exact atlas](https://github.com/algal/jacobianfun/tree/0a73d4c75bed60660c6e91a56f1595be756cbd59)

## Review record

This guide has not yet recorded an independent expert or machine check covering the whole claim.

## Limitations

- Historical priority over constructions not compared in the source.
- Any analogous result in dimension 2.

## Sources

- [A Counterexample to the Jacobian Conjecture](https://www.ulam.ai/research/jacobian.pdf) — primary
- [Hello there the Jacobian conjecture is false](https://x.com/__alpoge__/status/2079028340955197566)
- [jacobianfun — RESEARCH.md and exact checkers](https://github.com/algal/jacobianfun/tree/0a73d4c75bed60660c6e91a56f1595be756cbd59)

[Back to the claim inventory](../claims-v3.md)
