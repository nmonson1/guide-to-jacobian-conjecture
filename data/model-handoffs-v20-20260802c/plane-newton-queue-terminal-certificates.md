...
- the six-polynomial toric system is empty via its 296-point special fibre
  and Nakayama; and
- the exact stored layer-five-through-seven transformed system has no solution
  ([`RMU-6D8E0012`](../working-mathematics/units/RMU-6D8E0012.md)). This certificate does not establish that a full-support
  queue node reaches that stored system: the former canonical bare-`k=4`
  bridge is false because that shear begins at layer seven, not layer four.

The older 7,121-row minor is not retained as a theorem because its source does
not establish that the target basis is complete. Branchwise certificates are
the preferred terminal interfaces.
...
Starting from a hypothetical minimal plane Keller counterexample, the proposed
route runs an exhaustive normalized Newton queue to a terminal complete-chain
system.  The missing global theorem would show that every terminal system
either fails simultaneous finite polynomial support in adjacent boundary
charts or is induced by an admissible polynomial approximate-root operation
that strictly lowers Newton complexity.  With an exhaustive starting
reduction and termination, that dichotomy would exclude a minimal
counterexample.

Exact calculations rule out several direct affine-plane, polynomial-graph,
linear-target and low-parameter Hessian descents of the displayed known
higher-dimensional examples.  They do not exhaust all descents, prove queue
completeness, construct the adjacent charts, or prove the complexity-lowering
dichotomy.  See [`RMU-6D8E0011`](../working-mathematics/units/RMU-6D8E0011.md) and the complete scoped note and checkers in
[`research-notes/planar-descent-no-go-20260802-v1/README.md`](lane-8-source-packet.md).

## Exact live problem

Complete the **full-support** root of the queue. The truncated-support root is
already closed by [`RMU-6D8E0010`](../working-mathematics/units/RMU-6D8E0010.md). Starting from the public reconstruction
program, record the full-support node's normalized coefficients, fifteen
equations, valuation/deficiency layers and open locus. Then route every
coefficient and pivot branch until either a certified terminal system is
reached or the first complementary branch is missing. In particular, do not
attach [`RMU-6D8E0012`](../working-mathematics/units/RMU-6D8E0012.md) without an explicit chart map carrying the full-support
equations and open conditions to its stored transformed system.

## Tasks and deliverables

### P6-L8A — Full-support routing and first missing edge
...
[`RMU-6D8E0010`](../working-mathematics/units/RMU-6D8E0010.md), [`RMU-6D8E0012`](../working-mathematics/units/RMU-6D8E0012.md), their reconstruction/certificate packets, the
three retained dessin units, and the linked terminal proof sources.

Deliverable: a node/edge table rooted at the full-support system actually
generated by the public program, with hashes for its starting data, and the
first exact point at which an exhaustive queue edge is absent. Record the
truncated root as closed rather than re-auditing it. A full DAG is required
only if every full-support branch is supplied; otherwise the missing parent,
open locus, and unrouted complement are the result.
...
- [`RMU-6D8E0012`](../working-mathematics/units/RMU-6D8E0012.md) certifies its stored equations, not their provenance from the
  full-support root.
