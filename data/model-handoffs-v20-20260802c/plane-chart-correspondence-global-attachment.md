...
The corrected stored-system unit [`RMU-6D8E0012`](../working-mathematics/units/RMU-6D8E0012.md) supplies a useful terminal
certificate on the Lane 8 side, but it supplies no bridge from the
full-support root. Constructing such a bridge would require exactly the kind
of adjacent-chart realization studied here; the certificate itself cannot be
used to infer that the chart exists.
