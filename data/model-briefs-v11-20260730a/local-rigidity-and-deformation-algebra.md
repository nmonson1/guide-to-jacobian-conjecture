...
The newest exact finite inputs are tangent rank `439`, nullity `44`, and a
28-dimensional residual character; the character is not supported only in
weights `-2,-1`. Escaping-support and weight-three overlap calculations are
useful normal-cone evidence. A separate proposal proves saturation only at a
fixed nonzero source shear; its referenced certificate is unavailable, and
the exceptional and varying-shear loci remain open. Do not turn that local
proposal into an all-bending theorem.
...
- **One fixed nonzero shear proves global saturation.** It does not control
  exceptional shears, arcs varying the shear, or the missing transverse
  directions.
...
The new degree-eight tangent and character calculations are exact finite
inputs; the normal-cone conclusion is a separate geometric obligation, and
source-less saved outputs are not replay receipts.
