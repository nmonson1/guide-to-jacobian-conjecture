...
**Research state:** mathematical checkpoint 30 July 2026. A full
model-generated v13 audit found no counterexample to the main theorem but
identified one definite statement correction and several proof-completeness
or definition gaps. Exact scope is stated per input below.
...
[working paper](../../assets/manuscripts/{{MANUSCRIPT_04}}),
...
would record how an orbit degenerates, not all fppf equivalences. Likewise
the boundary stack
...
| --- | --- | --- |
| 1 | In the quadratic cubic-frame slice, for `alpha != 0`, `q=beta/alpha^2` is complete under arbitrary polynomial left-right equivalence and affine stabilization; the entire `alpha=0` line is one separate orbit. | [`JCG-0F9A20C0`](../../claims/JCG-0F9A20C0.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_04}}#page=2) |
| 2 | For coprime admissible cubic frames with fixed finite boundary length, stable equivalence equals ordinary equivalence and is classified up to scaling by `(Z_A,B|Z_A)`; `A` need not be squarefree. | [`JCG-B858C93E`](../../claims/JCG-B858C93E.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_04}}#page=12) |
| 3 | For admissible cubic frames with `A/c` nonconstant, the same fixed-frame Torelli conclusion holds across common roots and multiplicities, with the weighted relative-Jacobian divisor recovering the multiplicities. | [`JCG-E48F1FF0`](../../claims/JCG-E48F1FF0.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_04}}#page=16) |
...
| 10 | For the exceptional quadratic member `q=-2`, the weighted relative-Jacobian divisor has intrinsic vertical multiplicity exactly two, which separates the exceptional stratum uniformly. | [`JCG-7FB01BFA`](../../claims/JCG-7FB01BFA.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_04}}#page=17) |
...
**(F0) Proof hardening after the v13 audit.** The full model-generated audit
found no counterexample to the main stable-classification theorem, but it is
not a human review receipt. Direct source inspection confirms the elementary
`e>=1` correction, the stabilized `Fitt_(m+1)` index, the admissibility reason
that `b_1=0`, and the candidate status of the graph-closure paragraph. Still
needing independent proof review are the projective-incidence open immersion
in full generality, empty deleted-scheme edge cases, the all-multiplicity
Torelli reconstruction, the `S_3` monodromy argument in the reciprocal
family, and the categorical-support argument.

**(F1) Define the local graph models, then glue them.** The one-root wall and
length-`m` principal-part coordinates are explicit, but the claimed graph
closures are not yet self-contained constructions. First define the base
ring, coordinates, graph map, closure, and degeneration functor and prove the
one-root identification. Then address overlap
...
New exact coefficient-side tests make this frontier more concrete. The
normalized-multigraph packet gives a 12-equation saturated `N=3` Cox graph
and a fan refinement with 66 simplicial cones covering all 28 original cones
while retaining all 14 rays; direct and ordered charts differ by
`Bl_(u^2,v)`. A second packet replays a recursive principal-parts star law
through its tested orders and an `m=3` common-factor atlas. These are local
construction tests, not a global stable quotient or Rees--Jordan theorem;
the referenced Rees--Jordan checker remains unavailable.
...
The hidden-kernel comparison remains conditional on fixed-frame Torelli and
a boundary exact sequence. More sharply, a dual-number hidden automorphism
shows that literal descent for the unrigidified family is false. A correct
theorem must rigidify or retain that stabilizer as stack structure. Proposed
formal-inertia and stabilized-automorphism descriptions have no independent
proof receipt and belong in F0's audit queue.
...
**Last formula replay:** 30 July 2026. The attached audit calculation
reproduces component degrees `(11,10,4)`, formal source-degree triples
...
- **The unrigidified family descends through hidden infinitesimal
  automorphisms.** The dual-number example refutes literal descent; retain
  the stabilizer or state the rigidification.
...
*Inputs:* this page; the [Program 4 paper](../../assets/manuscripts/{{MANUSCRIPT_04}});
and the all-multiplicity, reciprocal-family, and categorical-boundary
appendices.

*Payoff:* turns the current credible-but-compressed proof chain into a
referee-auditable one and tests every proposed repair independently.

*Attack:* verify the projective incidence/open-immersion and proper-boundary
steps, the stabilized Fitting index and all-multiplicity reconstruction, the
reciprocal `S_3` argument, and the categorical-support theorem. Check every
repair against the actual hypotheses rather than assuming the audit is
authoritative.

*Done when:* every theorem has a complete proof locator, every edge case is
owned, and a second complete proof or targeted counterexample search has
tested the repaired argument, with any remaining gaps explicit.
...
the public categorical-wall checker, and the explicit one-root transition.
...
The 30 July v13 proof audit is a model-generated evidence lead, not a human
or specialist receipt. Its symbolic program replays successfully and checks
only the printed formulas and degree counts. Use its prose to target P4-T1,
retaining the distinction among definite statement defects, plausible proof
repairs, and constructions that are not yet auditable.
