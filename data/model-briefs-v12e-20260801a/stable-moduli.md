...
not a human review receipt. The source now records the elementary `e>=1`
correction, the stabilized `Fitt_(m+1)` index, the admissibility reason that
`b_1=0`, a proper-boundary argument, and the candidate status of the
graph-closure paragraph. Still needing independent proof review are the
projective-incidence open immersion in full generality, empty deleted-scheme
edge cases, the all-multiplicity Torelli reconstruction, the `S_3` monodromy
argument in the reciprocal family, and the categorical-support argument.
The newest independent audit reached the same definite `e>=1` and
`Fitt_(m+1)` corrections and additionally flags A.3, C.3, and D.3 proof
gaps. No repair artifact accompanied that audit, so these remain review
targets rather than locally applied manuscript repairs.
...
its portable checks replay. Direct and ordered charts differ by
`Bl_(u^2,v)`. The Lane 2 packet independently replays a recursive
principal-parts star law through its tested orders and an explicit `m=3`
common-factor atlas. These are exact local/coefficient calculations, not a
stable-quotient or global Rees--Jordan theorem. The referenced
`rees_jordan_checks.py` and output remain unrecovered.
...
reciprocal `S_3` argument, and the categorical-support theorem. Check the
audit's proposed repairs against the actual hypotheses; do not copy them
blindly.
