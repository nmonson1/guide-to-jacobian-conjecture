...
**Research state:** content checkpoint 29 July 2026, including the stored
layer-seven exclusion and the exact rational `F_2` jet through order 520.
Verification and review status is stated per input below.
...
explicitly incomplete for the global below-125 implication. “Accepted as
project input” means the exact finite or geometric statement may be reused
at its cited scope; it does not mean externally reviewed. No Program 6 input
below has independent review recorded. The open conditional lower-bound
claim is listed as a dependency, never as a proved input.
...
The face-to-Belyi package is `recorded`; the normal-form, layer, rechart,
terminal, Puiseux-correction, and slice results are `proof offered`; the
finite-support and `F_2` calculations are `certificate offered` or recorded
at their named scope. These are project evidence statuses, not review.
