...
**Research state:** content checkpoint 29 July 2026, including the two
zero-result large-CAS attempts and the exact six-dimensional compression
transversal. Verification and review status is stated per input below.
...
assigning the two open geometry problems. “Accepted as project input” means
use the cited statement at its exact, often tensor-relative scope. It does
not mean externally reviewed: no Program 5 input below has independent
review recorded. Finite-field audits and failed large-CAS runs retain their
weaker statuses and are never promoted by proximity to exact certificates.
...
Items 2, 3, 5's Hessian type, and 6's pairing interval include `recorded`
inputs; the Laplacian identity is `certificate offered`; the remaining
positive results are `proof offered` at their stated scopes. None has
