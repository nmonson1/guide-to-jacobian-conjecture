...
explicit: a future global queue would route every minimal counterexample to
a terminal complete-chain system, while this lane would have to prove
simultaneous two-sided finite support impossible or realize an admissible
complexity-lowering rechart. The local no-go calculations attached to that
strategy do not prove either global step.

The direct Lane 8 theorem [`RMU-6D8E0013`](../working-mathematics/units/RMU-6D8E0013.md) closes both normalized `(8,28)`
support roots and yields the strict below-`125` corollary [`RMU-6D8E0014`](../working-mathematics/units/RMU-6D8E0014.md)
without any adjacent-chart bridge. The corrected stored-system unit
[`RMU-6D8E0012`](../working-mathematics/units/RMU-6D8E0012.md) therefore remains an exact but unattached test system, not a
dependency of that theorem. Lane 9 begins beyond the solved finite bound: it
asks which ambient wall transports come from actual complete-chain charts
and, ultimately, how the degree-`125` `F_2` endpoint neighborhoods attach.
...
- The terminal-descent proposal [`RMU-6D8E0011`](../working-mathematics/units/RMU-6D8E0011.md) is an incomplete proof
  strategy, not a proved global attachment theorem.
- The direct below-`125` closure [`RMU-6D8E0013`](../working-mathematics/units/RMU-6D8E0013.md) does not solve this chart
  realization problem; conversely, no conjectural chart bridge should be
  inserted into that direct closure.
