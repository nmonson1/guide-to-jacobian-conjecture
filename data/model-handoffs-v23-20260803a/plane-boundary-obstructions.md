...
[working paper](../../assets/manuscripts/{{MANUSCRIPT_06}}),
...
| --- | --- | --- |
| 1 | Over characteristic zero, a primitive monomial Newton-face equation with the endpoint degrees and normalization stated in the paper has the displayed logarithmic derivative; after quotienting by the exponent-lattice gap, its rational function is Belyi with the stated exponent-determined passport. | [`JCG-9276A5DB`](../../claims/JCG-9276A5DB.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_06}}#page=5) |
| 2 | On one smooth boundary component and before support restrictions, the exact change `(A,B,s) -> (H,W,T)` displayed above conjugates the determinant equation to the rank-one normal form and decouples every normal order. It need not preserve sparse Newton windows. | [`JCG-D5FD24E3`](../../claims/JCG-D5FD24E3.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_06}}#page=19) |
| 3 | For the full `(8,28)` coefficient windows and the precise operator `D_r^{2,3}`, the normal-layer maps are injective for `r>=5`; for `5<=r<=11`, `dim coker D_r=r-1`. | [`JCG-79E508CE`](../../claims/JCG-79E508CE.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_06}}#page=22) |
| 4 | Every finite formal two-variable Jacobian-one jet is realized to that order by a polynomial Jacobian-one automorphism generated by linear maps and shears. This is unrestricted finite-jet realization, not realization in the filtered approximate-root subgroup. | [`JCG-D5D87491`](../../claims/JCG-D5D87491.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_06}}#page=23) |
| 5 | For the stored degree-21 lower-face equation `pq+2zp q'-3zp' q=1`, there are exactly five normalized connected dessin classes; they form one irreducible quintic Galois orbit and each has monodromy `A_21`. | [`JCG-2B32290C`](../../claims/JCG-2B32290C.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_06}}#page=24) |
| 6 | In the stored terminal face, the one-dimensional layer-four residual direction is the `k=4` complete-chain rechart `Y -> Y+lambda X^-4`, not a fixed-chart automorphism; the adjacent-chart Keller layers force the displayed common approximate root. | [`JCG-9667172F`](../../claims/JCG-9667172F.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_06}}#page=29) |
| 7 | After that canonical rechart and forced adjacent-chart condition, the complete stored degree-21 layer-five-through-seven support and matching equations have no common zero over the algebraic closure of the specified quintic field. This is one stored terminal system, not queue exhaustiveness. | [`JCG-BCEB7F45`](../../claims/JCG-BCEB7F45.md) · [exact proof](../../assets/manuscripts/{{MANUSCRIPT_06}}#page=30) |
| 8 | For the displayed normalized finite systems, the two exceptional ideals contain `1`. The selected six-polynomial toric system has the certified 296-point special fiber, and properness, finiteness, and Nakayama show that it has no characteristic-zero solution. The separate 7,121-row minor proves a unit ideal only if replay confirms that its rows are the complete target basis containing `1`. No other support system is covered. | [`RMU-ED13585F`](../working-mathematics/units/RMU-ED13585F.md), [`JCG-8917AD05`](../../claims/JCG-8917AD05.md), [`JCG-E91A93EB`](../../claims/JCG-E91A93EB.md) · [unit-ideal proof](../../assets/manuscripts/{{MANUSCRIPT_06}}#page=27) · [toric proof](../../assets/manuscripts/{{MANUSCRIPT_06}}#page=28) |
| 9 | In the exact `C_5`-invariant `F_2` recurrence with every new kernel coordinate retained, later parameters cancel both apparent external conditions at orders 510 and 520 over `Q`; neither value is a terminal invariant. The sliced order-530 value is likewise not global. | [`JCG-2533E53C`](../../claims/JCG-2533E53C.md) · [exact calculation record](../../assets/manuscripts/{{MANUSCRIPT_07}}#page=29) |
...
**Paper-audit checkpoint, 30 July 2026.** The repair packet is now integrated
into the canonical Program 6 source. It adds the missing residue/order
assumptions to Proposition 5.2 while retaining `e=17`, fixes Proposition
3.3's codomain, supplies a hand proof for `A_21`, replaces C.8's finite-etale
step by finite plus Nakayama, and makes the 7,121-row Macaulay implication
conditional on a complete target basis containing `1`. The independent
symbolic, quintic-arithmetic, and Hurwitz scripts replay. None closes queue
exhaustiveness or attachment, so the below-125 statement remains conditional.
The original public corrigendum and three small-check programs remain pinned on
[draft PR 4](https://github.com/nmonson1/guide-to-jacobian-conjecture/pull/4)
at `0b378effc3ac06f8aa07c77aaf05a827cc803d9f`; its obsolete generated-site
tree was not merged and it does not contain the large terminal replays.
...
| 8 | Reconstruct every terminal equation from principal-part residues, reduce at a good prime over 2053, enumerate the 296-point toric special fiber, and use properness plus finiteness and Nakayama to lift nonvanishing of the sixth obstruction. **Output:** unit-ideal and toric certificates with residue provenance. | Exact only for the displayed finite systems and their recorded support hypotheses; the older 7,121-row minor still needs its complete-target-basis semantics replayed. |
...
A useful formalism from the newest Program 9 conversation is the filtered
action complex `g_adm -> E -> W` with obstruction quotient
`ker D/(gauge + recharts)` and exact transport of quotient duals and forcing
terms. The decisive missing definition is `g_adm`: the full Laurent module
surjects, ordinary affine-polynomial fields give the wrong ranks, and the
complete-chain approximate-root subgroup must be built explicitly. A
reusable package and tests were reported externally but no package artifact
was attached to the conversation. The package and a larger Program 6
chart-correspondence packet were subsequently recovered on
[public-site draft PR 1](https://github.com/nmonson1/guide-to-jacobian-conjecture/pull/1)
at `d07d7bf5684967f6e4f698dd58a38b52215447b7`; all eight package tests
and all thirty Program 6 research-note tests pass locally. Treat it as
runnable research tooling for supplied finite-dimensional contracts, not as
a proof that the correct complete-chain subgroup or exhaustive chart list
...
The new Lane 9 toolkit adds exact Lie-bracket, general wall-closure, and
residual-kernel bookkeeping tools, and all six shipped unit tests replay.
Its upper-face atlas is built from transcribed public replay counts rather
than the actual matrices; the lower-face operation subgroup is incomplete;
the archived `C9`-specific wall replay is unavailable; and the actual `F_2`
blocks are absent, so the included `F_2` example is synthetic. Its averaging
lemma is nevertheless strategically useful: plain cyclic descent cannot by
itself obstruct the stable-support linear system. The next F4 attack must use
the real blocks plus global support, nonlinear coupling, or a stronger
descent condition.
...
- **Conductor forces polynomial termination.** Conductor information can
  ...
  terminate.
- **Synthetic `F_2` blocks certify the actual attachment.** They test the
  workflow only. Recover or reconstruct the actual blocks and retain the
  averaging lemma's warning that plain cyclic descent is insufficient.
...
The July 30 Lane 9 bundle is a supplemental toolkit, not another terminal
receipt. Its tests validate its internal wall, Lie, support, and symbolic
utilities. The missing matrices and archived special replay prevent its
structural interpretations from upgrading the proved list above.

The later filtered-action package is recovered on
[public-site draft PR 1](https://github.com/nmonson1/guide-to-jacobian-conjecture/pull/1).
Its eight package tests and thirty Program 6 research-note tests pass
locally, including the synthetic Program 5 interface and the Program 6
adapter. The actual Program 5 109/75 matrices are not imported, and the
complete-chain admissibility and global chart/attachment theorems remain
open; therefore the package supplies an implementation receipt, not a new
terminal theorem.

The attached raw-support reconstruction summary is a fresh exact replay:
the truncated support has sizes `25/47` and Macaulay rank 14; the full
support has sizes `61/125`; its weight-five-through-eight equation counts are
`1,3,5,6`; and all fifteen normalized equations match the legacy oracle.
This closes a row-generation/replay gap, not the upstream exhaustiveness or
descent gates.

**Last replay-audited:** `2026-07-29T19:26:14Z`; sources and inputs match the
pinned contract, and all three generated JSON outputs are byte-identical.
...
- Do not infer curvature or global obstruction geometry from the exact Lie
  brackets without the missing comparison theorem and actual data blocks.

[Back to the Program 6 overview](../../research/programs/plane-boundary-obstructions.md)
