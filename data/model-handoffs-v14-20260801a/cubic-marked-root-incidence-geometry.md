...
alone. The [retained working graph](../working-mathematics/programs/cubic-marked-root-incidence-geometry.md),
[current text proof sources](../proof-sources/01-cubic-incidence/main.md), and
[technical materials](../../evidence/materials.md#1-cubic-marked-root-incidence-geometry)
give current statement, proof, context, and replay access without private
conversations or internal paths. The linked PDF is an older optional reading
format, not the authority for August 1 repairs.
...
four cases, `U0`, `U1`, `U2`, and `B`; the July 31 working source repairs the
`U0` omission in the linked v12 paper. The other cases respectively include three completed sheets with one
...
│     ├─ candidate smooth-axis MCM module ── comparison still required
...
| --- | --- | --- |
| 1 | For a polynomial potential `H(T,c)` over a characteristic-zero field, the displayed root-slope substitution has Jacobian `-2` whenever its pole-cancellation conditions make it polynomial; its generic degree equals the root degree of `H`. | [`JCG-0106262F`](../../claims/JCG-0106262F.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_01}}#page=15) |
| 2 | For normalized cubic frames, polynomial left-right equivalence is exactly affine change of `c`, the prescribed scalar relation on `A`, congruence of `B` modulo `A`, and preservation of the marked root. Repeated roots are allowed. | [`JCG-221985FF`](../../claims/JCG-221985FF.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_01}}#page=7) |
| 3 | In the two-block multiplication-incidence construction with `a,b >= 1` and `a+b >= 3`, stable affineness occurs exactly for the unordered pair `{a,b}={1,2}`. | [`JCG-517DA8F4`](../../claims/JCG-517DA8F4.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_01}}#page=11) |
| 4 | For a coprime normalized cubic frame, the inverse cubic is irreducible with nonsquare discriminant, so generic monodromy is `S_3`; the target deck group is trivial. | [`JCG-504733CF`](../../claims/JCG-504733CF.md), [`JCG-5B09E55B`](../../claims/JCG-5B09E55B.md) · [proof](../../assets/manuscripts/{{MANUSCRIPT_01}}#page=5) |
| 5 | For the finite normalization attached to a generic-degree-three complex Keller map, the corrected divisorial taxonomy is `U0`, `U1`, `U2`, or `B`; a deleted three-cycle is impossible. This is divisorial only. | [`RMU-C1DD871B`](../working-mathematics/units/RMU-C1DD871B.md) · [current proposition and proof source](../proof-sources/01-cubic-incidence/appendices/cubic-resolvent-defects.md#label-prop-cubic-divisorial-trichotomy) |
| 6 | The finite cubic normalization splits as `O ⊕ E` with `E` rank-two reflexive, and its nonflat locus is finite and contained in omitted values. Reflexivity alone does not imply flatness. | [`JCG-C04524CF`](../../claims/JCG-C04524CF.md), [`JCG-A5938472`](../../claims/JCG-A5938472.md) · [proof and defect boundary](../../assets/manuscripts/{{MANUSCRIPT_01}}#page=13) |
| 7 | If a cubic defect is encoded by the quadratic-resolvent eigensheaf, it cannot be supported only at an isolated resolvent singularity; nontrivial three-torsion is detected at height-two singular primes. | [`RMU-13177F24`](../working-mathematics/units/RMU-13177F24.md) · [current theorem and proof source](../proof-sources/01-cubic-incidence/appendices/cubic-resolvent-defects.md#label-thm-dao-codim-two) |
| 8 | If one constructs a rank-one reflexive three-torsion module `M` on the quadratic resolvent, proves it is MCM, and matches its class with the normalization eigensheaf at every height-two singular prime, then codimension-two detection identifies the two modules and the cubic normalization is flat. A smooth cubic-axis picture does not by itself establish these hypotheses. | [`RMU-A4B084FF`](../working-mathematics/units/RMU-A4B084FF.md) · [current proposition and proof source](../proof-sources/01-cubic-incidence/appendices/cubic-resolvent-defects.md#label-prop-smooth-axis-comparison) |
...
| 8–9 | For the resolvent route, construct an actual rank-one three-torsion MCM module and compare its height-two classes with the normalization eigensheaf; Dao's torsion exclusion then gives the exact conditional flatness conclusion. Separately, the companion smooth-leading-cubic calculation gives a formal gauge model and an exact local sign-torsor kernel `F_3^2`. **Output:** an exact MCM comparison target and a residual local torsor space. | Neither the MCM construction/comparison nor global survival of a local torsor is supplied. |
...
a cube/conormal root becomes MCM. The smooth-axis proposal is a source of candidate modules, not
...
*Inputs:* this page; the [current Program 1 text sources](../proof-sources/01-cubic-incidence/main.md), especially the cubic-resolvent defect appendix; the precise retained units in F1–F2.
...
the smooth-axis proposal with the singular height-two primes; keep the central
...
The text source pages hold the current conventional proofs and context. The
public Program 1
...
Proof access is organized by retained-unit pages and exact text-source label
anchors. PDFs are optional archival reading copies. Start with the
...
- Do not claim the `U0/U1/U2/B` divisor classification controls isolated
  nonflat points.
