---
title: "Invariant ring at a transverse simple conic basepoint"
description: "Let k be a field of characteristic zero. Suppose p is a common zero of A and B such that dG(p), dA(p), and dB(p) are linearly independent. In etale local coordinates (g,a,b)=(G,A,B), let delta=-2g partial_g+a partial_a+b partial_b. Then k[g,a,b]^delta=k[ga^2,gab,gb^2], which is isomorphic to k[U,V,W]/(V^2-UW). In particular, a transverse simple basepoint contributes no additional local polynomial invariants beyond the three conic target coordinates."
---

# Invariant ring at a transverse simple conic basepoint

`RMU-B4A5E001` · `lemma` · statement version `1`
...
- The etale-local transverse simple-basepoint model for a fixed-factor conic leading form.
...
### The source supplies a complete monomial-weight proof of the local invariant-ring identity.

`SUP-RMUB4A5E001-01` · `proof`

The source supplies a complete monomial-weight proof of the local invariant-ring identity.

**Establishes:** The displayed invariant-ring equality and its conic relation under the stated etale-local hypotheses.
...
- A global degree-five fixed-factor exclusion.
- Any statement about nontransverse or higher-length basepoints.

## Mathematical connections

- `related_to` [`JCG-F299193A`](JCG-F299193A.md) — This local calculation narrows the retained open basepoint-boundary problem to nontransverse, higher-length, and global-gluing cases.
