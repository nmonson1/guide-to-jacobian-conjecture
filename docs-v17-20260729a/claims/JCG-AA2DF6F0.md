---
title: "Comparison with Vitushkin's 1999 rational example"
description: "Vitushkin's 1999 rational example has an analogous multiplication-map presentation and is generically two-to-one, but its source is a punctured plane, so the resulting map has a pole."
---

<p class="claim-tag">JCG-AA2DF6F0</p>
# Comparison with Vitushkin's 1999 rational example

<p class="dek">Vitushkin's 1999 rational example has an analogous multiplication-map presentation and is generically two-to-one, but its source is a punctured plane, so the resulting map has a pole.</p>

<span class="status status-kind">Claim</span> <span class="status status-draft">Proof offered — review pending</span> <span class="status">Headline</span>

## Exact statement

Vitushkin's 1999 rational example has an analogous multiplication-map presentation and is generically two-to-one, but its source is a punctured plane, so the resulting map has a pole.

Statement version `1`. The public tag is stable; statement revisions increment the version rather than replacing the tag.

## Appears in

- [Why the Double-Root Slice Is Affine Three-Space](../collections/double-root-affine-source.md) — shared, supporting result
- [Comparison with Vitushkin's 1999 Rational Example](../collections/vitushkin-rational-comparison.md) — defining, primary statement

## Proof access and evidence boundary

- [Why the Double-Root Slice Is Affine Three-Space](../collections/double-root-affine-source.md): **Locator audit incomplete**
- [Comparison with Vitushkin's 1999 Rational Example](../collections/vitushkin-rational-comparison.md): **No program manuscript claimed**

**Independent review**

- None Recorded: No independent review is represented in this public record.

**Recorded evidence**

- Literature Result: Tao's comparison with the rational example identified in the comment thread.

## Public sources

- [Anonymous's comment — Anonymous](https://terrytao.wordpress.com/2026/07/21/a-digestion-of-the-jacobian-conjecture-counterexample/comment-page-1/#comment-693534)
- [Terence Tao's comment — Terence Tao](https://terrytao.wordpress.com/2026/07/21/a-digestion-of-the-jacobian-conjecture-counterexample/comment-page-1/#comment-693536)

## Credit

- Anonymous commenter 693534: research assistance; attributed by source — pointed to Vitushkin's 1999 example
- Terence Tao: derivation, exposition; attributed by source — worked out and stated the multiplication-map comparison

[Browse all claims](../results/all-claims.md)
