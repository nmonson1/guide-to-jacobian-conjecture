---
title: "Stable Deformation Moduli"
description: "For the quadratic cubic-frame slice with alpha nonzero, q=beta/alpha^2 is a complete invariant under arbitrary polynomial source-target equivalence and stabilization; the alpha=0 line is gauge-equivalent to the base map."
---

# Stable Deformation Moduli

<p class="dek">This historical package includes replaced atomic statements. Follow the current-unit links below for reusable mathematics.</p>

<span class="status status-kind">Result</span> <span class="status coverage-complete">Exact manuscript location</span>

## Precise statement

This package contains one or more replaced atomic statements. The current mathematical statements and forward links are listed below.

## Claims in this result package

### [JCG-10B255A3 · The based w20 family has exactly two based equivalence classes, k=0 and k nonzero, while its coefficientwise formal trivializations require unbounded spatial degree.](../claims/JCG-10B255A3.md)

The based w20 family has exactly two based equivalence classes, k=0 and k nonzero, while its coefficientwise formal trivializations require unbounded spatial degree.

*Defining · Subsumed Theorem · Recorded*

### [JCG-0F9A20C0 · For the quadratic cubic-frame slice with alpha nonzero, q=beta/alpha^2 is a complete invariant under arbitrary polynomial source-target equivalence and stabilization; the alpha=0 line is gauge-equivalent to the base map.](../claims/JCG-0F9A20C0.md)

For the quadratic cubic-frame slice with alpha nonzero, q=beta/alpha^2 is a complete invariant under arbitrary polynomial source-target equivalence and stabilization; the alpha=0 line is gauge-equivalent to the base map.

*Defining · Primary Statement · Proof supplied*

### [JCG-5542AF40 · For squarefree A and marked missing-infinity roots, stabilized isomorphism classes of normalized conductor arrangements are exactly classified by affine changes of c, affine-linear changes of t over c, the affine root configuration of A, and the projective vector of conductor values B(rho) modulo induced finite permutations.](../claims/JCG-5542AF40.md)

For squarefree A and marked missing-infinity roots, stabilized isomorphism classes of normalized conductor arrangements are exactly classified by affine changes of c, affine-linear changes of t over c, the affine root configuration of A, and the projective vector of conductor values B(rho) modulo induced finite permutations.

*Shared · Supporting Result · Proof supplied*

### [JCG-C8A7EF37 · Growing Stable Moduli in the Cubic Frame](../claims/JCG-C8A7EF37.md)

For every n at least two, the cubic-frame construction contains a (2n-3)-dimensional family of pairwise stably inequivalent generic-degree-three Keller maps of ordinary degree 4n+3. In particular, ordinary degree fifteen contains a three-dimensional stable-moduli family, modulo a finite involution.

*Shared · Supporting Result · Proof supplied*

### [JCG-CA01A7A7 · Within the full cubic-frame family, ordinary degree eleven is the first degree at which a genuine positive-dimensional stable modulus can occur.](../claims/JCG-CA01A7A7.md)

Within the full cubic-frame family, ordinary degree eleven is the first degree at which a genuine positive-dimensional stable modulus can occur.

*Shared · Supporting Result · Proof supplied*

### [JCG-F7DC2C3B · Distinct q-families have the same first-order formal tangent and are source-trivial through the computed formal orders, while their punctured members remain stably inequivalent; the stable modulus is therefore invisible to this local formal deformation data.](../claims/JCG-F7DC2C3B.md)

Distinct q-families have the same first-order formal tangent and are source-trivial through the computed formal orders, while their punctured members remain stably inequivalent; the stable modulus is therefore invisible to this local formal deformation data.

*Shared · Supporting Result · Recorded*

### [JCG-2FF16912 · The Bare Stable-Orbit Moduli Problem Is Nonseparated](../claims/JCG-2FF16912.md)

For the quadratic cubic-frame family A=c+alpha c^2 and B=-2-4alpha c+beta c^2, the arcs alpha=t and beta=q t^2 have constant stable class q away from t=0 and specialize to the base class at t=0. Consequently no separated algebraic space can functorially classify the bare stable left-right orbit set while distinguishing these geometric classes.

*Shared · Moduli Obstruction · Proof supplied*

### [JCG-D1155E99 · Replaced historical record](../claims/JCG-D1155E99.md)

!!! warning "Replaced working statement"
    The historical wording is suppressed here. Use the [current replacement](../research/working-mathematics/units/RMU-3FEF0011.md).

For A_alpha(c)=c(1+alpha c), B_(alpha,q)(c)=-2-4alpha c+q alpha^2 c^2 over a commutative Q-algebra, a c-fixed framed root translation of c-degree at most D from q to q' exists exactly when (q'-q)alpha^(D+2)=0; it is unique and has residual (-1)^D(q'-q)alpha^(D+2)c^(D+2). For alpha=s modulo s^M and q!=q', the minimal framed degree is M-2. Over C[[s]], all Artin truncations are compatibly ordinarily left-right equivalent, but the complete families are not stably polynomially left-right equivalent. Their unrestricted stable-equivalence complexity diverges, with the explicit lower bounds in the proof.

*Shared · Rigidity Moduli Synthesis · Proof supplied*

## Evidence and manuscript boundary

The linked working manuscript has exact locations for every program-relevant defining claim on this page. This says where the claims are recorded, not that they have been independently verified.

Complete coverage requires an exact LaTeX location for every program-relevant defining claim. A locator records where the current statement and evidence boundary appear; it is not a proof-review status.


## Credit

- Nathaniel Monson: research direction and accountable author; attributed by source
- Nathaniel Monson: research direction and mathematical responsibility; attributed by source

## Connections

- [The Categorical Quotient of Bounded Cubic Frames](../collections/categorical-cubic-frame-quotient.md)
- [Boundary Jets and Stable Moduli in the Cubic Frame](../collections/cubic-boundary-jets-and-moduli.md)
- [Degree Seven Local Affine Rigidity](../collections/degree-seven-local-affine-rigidity.md)
- [Finite Triple Covers, Conductors, and Automorphisms](../collections/finite-triple-covers-conductors-and-automorphisms.md)

[Back to Results](../results/index.md)
