...
| P2 quartic reductions | Expand the scaled determinant arc; stratify by leading target span; use valuations and Hilbert–Burch normal forms to route span two into ramification and fixed-component leaves. The recovered degree-three archive now exactly excludes the displayed nonresonant `F_3/F_4` families with arbitrary lower binary terms. **Output:** an explicit quartic case tree plus exact nonresonant degree-three certificates. | The lost `F_3/F_4` branch-exhaustiveness elimination, `tau=0,-1`, `c=0`, dependent-syzygy charts, and global case-tree audit remain open. | [Program 2 exact inputs and proofs](minimum-degree-and-quartic-exclusions.md#3-reusable-inputs-exact-scope-and-proof-access) |
...
classification, one-root graph closures, and local principal-part models.
...
fixed-component calculations now replay. A recovered historical-style archive
exactly excludes the displayed nonresonant `F_3` and `F_4` families, including
the full arbitrary-lower-term `D_6`-to-`D_5` compatibility calculation. Its
thirteen deterministic outputs reproduce byte-for-byte. The sharp work has
moved upstream and sideways: derive the `F_3/F_4` branches exhaustively,
close the `tau=0,-1`, `c=0`, and dependent-syzygy charts, assign every
exceptional divisor, and independently audit the result. The alternative
marked chart and its `104/3` anchors remain useful coordinate cross-checks. In
addition, an exact companion/Jordan-chain construction supplies a
...
foundational sheaf argument has no independent specialist review, and the
repository already has stronger closures for the affected loci. In
parallel, turn the incrementally built reductions into one audited case tree
from leading-span strata to every terminal chart. This lane has the most
immediate theorem payoff: it either proves `D_min >= 5` or identifies a new
quartic candidate. Its interaction with Lane 5 may replace some chart work
only if the budget is intrinsic to the full left-right orbit.
...
degree. A restricted minimum over explicit cubic-frame moves is not enough:
the target is a monotone over arbitrary polynomial left-right changes, or a
normalization procedure reaching canonical form without increasing degree.
...
| 2 | Program 1 `P1-T3` flatness/opening separation; Program 4 `P4-T1` global graph boundary, `P4-T2` low-`N` overlap tests |
...
| 4 | Program 2 `P2-T1` degree-three branch/resonance completion, `P2-T2` case tree, `P2-T3` proof/code correspondence |
