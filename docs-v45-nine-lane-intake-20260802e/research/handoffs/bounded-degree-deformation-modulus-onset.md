...
equivalent, while no polynomial framed equivalence exists. The stronger
unframed stable conclusion uses the separately stated stable
`q`-classification; its effective complexity bound also uses the cited
parametric effective Nullstellensatz. Unit: [`RMU-3FEF0010`](../working-mathematics/units/RMU-3FEF0010.md); proof, dependency
audit and three exact checkers: [`research-notes/lane3-formal-effectivity/`](lane-3-source-packet.md).
...
Independently, remove or sharply weaken the load-bearing stable
`q`-classification in [`RMU-3FEF0010`](../working-mathematics/units/RMU-3FEF0010.md): prove directly that the two generic
fibres with `q!=q'` cannot be stably polynomially left--right equivalent, or
construct such an equivalence. The framed orbit cokernel and exact Artin
staircase are supplied, so a solution must address unrestricted source,
target and stabilization operations rather than repeat the framed recursion.
...
### P3-L3D — Stable effectivity of the quadratic modulus

Status: ready.

Inputs: [`RMU-3FEF0010`](../working-mathematics/units/RMU-3FEF0010.md), `formal_effectivity_theorem.md`, its explicit source
and target automorphisms, and the dependency audit in
[`research-notes/lane3-formal-effectivity/`](lane-3-source-packet.md).

Deliverable: an unconditional stable invariant separating `q` and `q'` on
the generic fibre, or an explicit stable polynomial equivalence. If using a
classification theorem, restate it with exact hypotheses and verify that the
displayed family satisfies them. Keep the unconditional framed staircase
separate from any conditional stable conclusion.
